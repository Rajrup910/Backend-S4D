...
% TODO before submission: full author list with middle names, ORCID iDs, IEEE membership
% grades (\IEEEmembership{Student Member, IEEE}), and the corresponding-author marker.
\author{Rajrup%
...
never measured \emph{per subgroup}. We build a seven-class HAM10000 classifier as an ablation
ladder in which every fitted component is fitted on validation or on $6{,}981$ out-of-fold
training predictions, never on test, and every new test quantity comes from one pass against a
pre-registered $19$-item analysis plan. A uniform soft-vote over six CNN backbones reaches
...
combination levers, among them a mixed CNN--transformer ensemble and a $30$-member fold-bagged
ensemble, are all negative, and we report that exhaustion as a result. What remains is subgroup
...
rule fitted out-of-fold raises escalation sensitivity to $0.831$, at a Number Needed to Biopsy
of $3.0 \rightarrow 6.2$ at a stated $3\%$ reference prevalence.
\end{abstract}
...
\subsection{Dermoscopic lesion classification}
Convolutional baselines on HAM10000 and ISIC are
well established~\cite{esteva2017,codella2018isic,tschandl2018ham10000}, and the
...
on a single benchmark split are typically of the order of one to three F1 points --- which,
as our confidence intervals show, is smaller than the sampling variation of the split itself.

\subsection{Class imbalance}
HAM10000 is dominated by melanocytic nevi ($\sim67\%$ of images), which makes accuracy an
actively misleading selection criterion. Standard remedies include re-weighting by effective
number of samples~\cite{cui2019classbalanced}, margin-based losses such as
LDAM-DRW~\cite{cao2019ldam}, and asymmetric focal variants~\cite{ridnik2021asl}. We train
both LDAM-DRW and ASL variants of our strongest backbone and find neither improves on the
cross-entropy baseline under effective-number weighting (Sec.~\ref{sec:results-ladder}).
A second standard remedy is post-hoc: logit adjustment by the label
prior~\cite{menon2021logitadjust}, whose probabilistic form dates to prior-shift correction
for changing class priors~\cite{saerens2002adjusting}. We report it as a negative result and
explain the null mechanistically in Sec.~\ref{sec:results-exhaustion}: effective-number
weighting has already applied the correction at training time, so applying it again
over-corrects.

\subsection{Calibration and uncertainty}
Modern single networks are systematically overconfident~\cite{guo2017calibration}.
Temperature scaling is the standard single-parameter remedy; matrix and Dirichlet
scaling~\cite{kull2019dirichlet} generalise it to full class-conditional recalibration. We
note in Sec.~\ref{sec:results-calibration} that our soft-vote ensemble is miscalibrated in
the \emph{opposite} direction, which is why the single-parameter remedy is not the right one
here. All of these are \emph{marginal} notions of calibration. Group-wise notions ---
multicalibration~\cite{hebertjohnson2018multicalibration} and its
relatives --- require the calibration property to hold within identifiable subpopulations, and
our per-band results are a direct empirical argument for adopting one: a globally
well-calibrated map can be, and here is, simultaneously under- and over-confident in
different age bands.
Selective classification --- learning to abstain --- has a long
history~\cite{elyaniv2010selective,geifman2017selectivenet}, and deep-ensemble
disagreement~\cite{lakshminarayanan2017deepensembles} and Mahalanobis feature
distance~\cite{lee2018mahalanobis} are common uncertainty signals. Our finding that the
latter two underperform the probability margin on an in-distribution test split is
consistent with their stated purpose: they are shift detectors, and there is no shift here.

\subsection{Conformal prediction}
Split conformal prediction~\cite{vovk2005algorithmic,angelopoulos2023gentle} converts any
score into finite-sample coverage guarantees. LAC~\cite{sadinle2019lac} minimises set size;
APS~\cite{romano2020aps} and RAPS~\cite{angelopoulos2021raps} trade size for conditional
validity. The distinction between marginal and class-conditional (Mondrian) coverage is
well known in the statistics literature~\cite{vovk2012conditional} but is, in our reading,
under-emphasised in medical imaging applications, where it is precisely the clinically
relevant distinction. Conditioning on a protected or clinically salient attribute rather than
on the label --- equalised coverage --- is due to Romano
\emph{et al.}~\cite{romano2020equalized}; our bipartite (age band $\times$ escalation
requirement) calibrator is an instance of their construction, and we cite it as such rather
than presenting it as new.

\subsection{Fairness and subgroup evaluation}
Skin-tone bias in dermatological AI is documented~\cite{daneshjou2022disparities}, and
Fitzpatrick-labelled corpora~\cite{groh2021fitzpatrick17k} exist to measure it. HAM10000
carries no Fitzpatrick labels, and we explain in Sec.~\ref{sec:limitations} why we decline to
approximate them from image statistics; we instead measure the skin-tone axis externally on
the Fitzpatrick-labelled PAD-UFES-20 cohort~\cite{pacheco2020padufes}
(Sec.~\ref{sec:results-external}). The age-stratified failure we report appears to be less
discussed than the skin-tone axis, despite being, in our data, far larger in magnitude.

\subsection{Hidden stratification and subgroup underdiagnosis}
The failure mode this paper is ultimately about has a name. Oakden-Rayner
\emph{et al.}~\cite{oakdenrayner2020hidden} define \emph{hidden stratification} as the
situation in which a clinically meaningful subclass of cases is systematically mishandled
while aggregate metrics remain healthy, and show that the effect is common, large and
invisible to the reporting conventions of the field. Seyyed-Kalantari
\emph{et al.}~\cite{seyyedkalantari2021underdiagnosis} give the closest prior result to ours
in medical imaging: chest radiograph classifiers that underdiagnose historically
underserved patient groups, again with no aggregate signal. The mechanism is a shortcut in
the sense of Geirhos \emph{et al.}~\cite{geirhos2020shortcut}: a feature that is predictive
in the training distribution and clinically wrong, of which the radiograph-site confound of
Zech \emph{et al.}~\cite{zech2018confounding} is the canonical medical instance. ``Young
patient, therefore nevus'' is a textbook example, and the training prior we report in
Sec.~\ref{sec:results-fairness} is its source.

Our contribution over this lineage is specific. That work establishes that hidden
stratification exists and evades aggregate metrics. We additionally (i) \emph{measure the
mechanism}, by separating a decision rule discarding available signal from a genuine loss of
...
the affected subgroup, which is a stronger statement than that the classifier is worse there;
and (iii) supply a mitigation with a priced clinical cost. The mitigation itself is a
group-specific decision threshold, which is exactly the post-processing construction of
Hardt \emph{et al.}~\cite{hardt2016equality}. We select it by expected clinical cost under a
...
checklist~\cite{tejani2024claim}, supplied as supplementary material with a per-item pointer
...
variability cannot be computed, because neither source dataset releases per-rater labels.
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.88\textwidth]{figure2_reliability.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figure3_decision_curve_analysis.png}
\caption{Decision curve analysis. Across the clinically plausible range of threshold
probabilities the ensemble's net benefit exceeds both the best single model and the
treat-all strategy. At $p_t = 0.10$: ensemble $0.1577$, ConvNeXt-Tiny $0.1559$, treat-all
$0.1034$.}
\label{fig:dca}
\end{figure}

Decision curve analysis~\cite{vickers2006dca} (Fig.~\ref{fig:dca}) confirms that the
ensemble carries positive net benefit over both a treat-all policy and the single best model
across the plausible threshold range.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figure4_risk_coverage.png}
\caption{Risk--coverage curves for the seven uncertainty scores on the test split. The three
probability-based scores (margin, MSP, entropy) are mutually indistinguishable; the
disagreement-based scores (mutual information, ensemble variance) are consistently worse,
and Mahalanobis feature distance is worst by a clear margin across the whole operating
range. The margin was selected on validation AURC, but its advantage over MSP and entropy
is not resolvable on test.}
...
why they occupy a separate, coverage-annotated block, and why Fig.~\ref{fig:riskcoverage}
...
(Fig.~\ref{fig:riskcoverage}), so this is a coin landing the other way rather than a
...
its average from \textsc{nv} and \textsc{df} while under-covering the escalating classes,
worst on \textsc{mel} at $0.796$; class-conditional calibration (orange) lifts every class
to or above target. LAC shows the same pattern more severely (\textsc{mel} $0.671$,
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figure6_abstention_tradeoff.png}
\caption{Aggregate abstention trade-off on the test split. Macro-F1 and escalation
sensitivity on the retained cases rise monotonically with the deferral rate while the count
of missed serious cases falls from $78$ to $36$. The curve is well behaved at every
operating point, which is precisely why the subgroup breakdown in
Table~\ref{tab:agegap} --- not this figure --- is what exposes the under-$40$ failure.}
\label{fig:abstention}
\end{figure}

\textbf{Abstention does not rescue it, and the safety net is thinnest where it is needed most.}
Fig.~\ref{fig:abstention} shows a well-behaved aggregate abstention trade-off --- retained-set
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figure7_gradcam.png}
\caption{Grad-CAM attributions for ConvNeXt-Tiny on the test split. Top row: the
highest-confidence correct case for four classes the model handles well; attribution is
compact and lesion-centred. Bottom row: the highest-confidence \emph{error} for four
classes it confuses, including both directions of the \textsc{mel}/\textsc{bkl}
confusion. Panels are selected deterministically by confidence from
\texttt{gradcam\_stats.csv}. The bottom row is the failure mode abstention cannot catch:
the attribution is on the lesion and the model is confident, but the label is wrong.}
\label{fig:gradcam}
\end{figure*}
...
HAM-fitted Dirichlet map on top makes matters worse still ($0.167 \rightarrow 0.133$,
escalation sensitivity $0.356 \rightarrow 0.223$), which is the expected behaviour of a
...
$0.223$ to $0.585$, melanoma recall from $0.115$ to $0.327$, missed serious cases from
$1{,}264$ to $675$, and Macro-F1 from $0.133$ to $0.177$. Read carelessly this looks like
...
remains outstanding. And even with the rule, Macro-F1 ($0.177$) stays below the best raw
single member ($0.188$): PAD is a different domain, not a rescued deployment.

\textbf{Mahalanobis distance, given the shift it was designed for, works.} Ranked last of
seven uncertainty scores on the in-distribution test split (Sec.~\ref{sec:results-selective}),
the same class-conditional Mahalanobis score --- fitted on HAM training features and not
refitted --- separates in-distribution HAM validation ($n=1{,}532$) from shifted PAD
($n=2{,}106$) at AUROC $0.913$, with median scores of $485$ against $8{,}717$, an $18\times$
separation. Its in-distribution failure was not a defect of the score but a mis-specified
task: there was no shift to detect. The two results together are the argument for pairing a
boundary-based score for in-distribution abstention with a density-based score for shift
detection, rather than choosing between them.
...
equalised-odds true-positive-rate gap across all groups is $0.297$, but that figure is
\emph{not} a skin-tone finding and must not be quoted as one: it is driven almost entirely by
the $804$ images with no Fitzpatrick label, whose escalation sensitivity is $0.051$ against
$0.256$--$0.349$ for every labelled type. That is a data-completeness artefact, and the
population with missing labels differs systematically from the population with them.
Restricted to types I--IV, the true skin-tone spread is $0.093$ and \emph{non-monotonic}
(I $0.349$, II $0.290$, III $0.256$, IV $0.278$). It does not reproduce the
...
\textbf{Single training dataset, and the cross-domain picture is worse than a single model
suggests.} All training is on HAM10000. Applied unmodified to PAD-UFES-20 smartphone
photographs, the six-CNN ensemble reaches Macro-F1 $0.167$ --- below its own best member
($0.188$) --- and the frozen calibration map degrades it further to $0.133$
...
correlated under shift. Nothing in this paper licenses pointing a dermoscopy-trained
classifier at phone photographs, and the external result is a shift benchmark, not a second
validation of the method.

\textbf{Skin tone is now partially measured, and remains open for darker skin.} HAM10000
carries no Fitzpatrick labels, and an individual typology angle proxy computed from image
statistics is invalid on dermoscopy --- vignetting and erythema dominate the measured angle
--- so we do not report one. The external cohort supplies real labels for types I--IV, where
the sensitivity spread is $0.093$ and non-monotonic. Types V and VI are represented by nine
images in total and are suppressed. This work therefore has nothing to say about darker skin,
which is the population where the published disparities are
largest~\cite{daneshjou2022disparities}, and answering that question requires a corpus that
contains it~\cite{groh2021fitzpatrick17k}.

\textbf{The out-of-fold protocol closes one problem and opens two.} The validation split no
longer does six jobs: calibration, thresholds, abstention quantiles and conformal quantiles
can now be fitted on $6{,}981$ out-of-fold rows, which quintuples the rarest class's
calibration support and eliminates every degenerate conformal cell at $\alpha=0.05$. It costs
something on both sides. Out-of-fold calibration is fitted by models trained on $80\%$ of the
data and applied to a model trained on all of it, a stacking mismatch we measure but cannot
remove, and rung A7-oof is $0.018$ Macro-F1 below its validation-fitted counterpart. And
out-of-fold conformal calibration forfeits the exact finite-sample guarantee, because the
calibration and test scores no longer come from one fixed score function; we audit its
coverage empirically and never assert it. Cross-conformal or CV+ would restore a
$(1-2\alpha)$ guarantee but requires scoring the test split with all five fold models, a
second test read that our protocol does not permit here.

\textbf{The fold-bagging rung is under-sharpened by construction.} Rung A8 reuses the
six-member out-of-fold Dirichlet map rather than one fitted for a $30$-member vote, and a
$30$-member vote is more hedged than a six-member one. Its negative result is therefore a
lower bound on fold-bagging, not a refutation of it. This caveat was pre-registered rather
than added after the number came back.

\textbf{The age rule's selection criterion is clinical cost, not equality of opportunity.}
Each $\lambda_b$ minimises expected cost under our cost matrix subject to a per-band
specificity floor. The principled objective for a group-specific threshold is equality of
opportunity~\cite{hardt2016equality} --- equal true-positive rates across bands --- which is
the same computation under a different criterion and would be the cleaner choice. We did not
make it, and we note that our under-$40$ $\lambda$ has a bootstrap interval touching zero, so
that band's parameter is the least certain of the three.

\textbf{The two safety mechanisms are not orthogonal in the band that matters.} At the $10\%$
operating point the age rule and the abstention gate rescue the same two of eighteen
under-$40$ misses (Table~\ref{tab:ortho}). They are complementary at $40$--$59$ and largely
redundant at $60+$. Sixteen confidently wrong under-$40$ misses are reached by neither
mechanism, and we have no proposal that reaches them. The intersectional slice
(Sec.~\ref{sec:results-intersectional}) narrows the affected population further than our
power gates allow us to characterise: the under-$40$ male cell misses the reporting floor by
a single escalating case.

\textbf{Colour constancy is not evaluated.} A Shades-of-Grey normalisation step was planned
and not implemented; we report its absence rather than leaving an empty row in the ablation.

\textbf{Rare-class support, correctly attributed.} An earlier version of this work attributed
the width of the Macro-F1 intervals and the ladder's unresolvability to \textsc{df} and
\textsc{vasc} scarcity. That conflates two different things and we separate them. The
...
$20$ and $21$ images. Rare-class scarcity widens the intervals; melanoma performance is what
holds the point estimate down.

\textbf{Attribution evidence is quantitative but confounded.} Lesion-interior attribution
fraction over all $1{,}502$ test images ($0.523$, concentration ratio $3.45$) replaces the
earlier $28$-image frame heuristic, but the metric is computed with respect to the predicted
class, so misclassified images score higher than correct ones. It supports a qualitative claim
about where the network looks and nothing stronger (Sec.~\ref{sec:results-gradcam}); the
mechanistic argument is carried by the escalation-mass AUC instead.

\textbf{Age is a proxy, and one test read means one test read.} We stratify by recorded
patient age because it is what the metadata contains; the underlying clinical structure ---
lesion presentation, nevus count, sun-exposure history --- is not observed, so ``under $40$''
is a label for a population, not an explanation of it. Finally, our protocol permits one
pre-registered pass over the test split. Every test quantity in this paper was named before
it was computed, and any analysis suggested by these numbers cannot be run against this test
split at all. That is the intended cost of the discipline, and it means several natural
follow-ups --- a same-modality replication of the age effect, CV+ conformal, a $30$-member
recalibrated fold bag --- belong to a future study with its own held-out data.
...
% TODO: acknowledgements, funding, compute, and dataset attributions before submission.
