...
of $3.0 \rightarrow 6.2$ at a stated $3\%$ reference prevalence. Carried frozen to two further
...
the standard that covers model transportability specifically, is supplied alongside it. Both
checklists are pointer indices into this paper rather than results, which is why they are
supplementary; the supplement also carries a qualitative atlas of the lesions the age rule
rescues.
...
(I $0.349$, II $0.290$, III $0.256$, IV $0.278$). Panel~B of
...
band rather than the worst. Bars are lesion-grouped bootstrap intervals. Both pre-registered
...
and argmax are fixed operating points and are therefore flat in $p_t$; only the risk model
traces a curve. \textbf{Left}: the full out-of-fold panel, where the rule dominates argmax
...
supplies real labels for types I--IV, where the sensitivity spread is $0.093$ and
