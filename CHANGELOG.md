...
per the project's Hard Rule 4, nothing here is hand-entered. Where earlier running logs and a
`results/` file disagreed, the `results/` file wins and the logs have been corrected alongside
this document.
...
correct, current numbers — the log below in §14 records that an earlier runbook entry had stale
...
- Stale runbook conformal numbers (89.3%/24, 92.4%/12 → 91.4%/17, 94.1%/6) were corrected in S0,
...
- **The PAD-UFES-20 checkpoints are gone, and the runbook claimed otherwise.** Its file map listed
  ...
  are not recoverable. **S8 must budget for retraining, not reloading** — corrected in the runbook
...
`DATASET_REFINING.md` and the ignore rules also cover local scratch docs. Both are project-internal
...
`research/plans/master-polymorphic-gadget.md` — *"V2 — Budget-Constrained Subgroup Safety:
...
written in session `ad81ff6c` from the owner's 49,035-character MASTER
...
project's governing spec lives in `research/plans/`, and `plan.py` calls it "the directive"
...
**What the next session must know.** The blueprint is at `research/plans/master-polymorphic-gadget.md`
...
# V4 — "moving toward solutions" (S48-S63)

### S48 — Power audit, endpoint declaration, seed policy: the endpoint rests on 10 lesions, not 21 (2026-09-15)

**No test read.** `results/test_pass_receipt.json` stays at **`n_executions: 2`**; frozen
checkpoints re-verified **6/6 byte-identical**. Only `research/v4/`, `results/v4/` and the ledger
were written. Source for every number below: `results/v4/power_audit.json` plus the three CSVs
beside it, all generated by `research/v4/power.py`.

**The finding S48 did not go looking for.** The runbook's premise is that under-40 escalation
sensitivity is estimated on 21 test positives and that pooling ISIC-2019 raises that to ~150. Both
halves of that sentence count **images**. Joining `ml/configs/splits/split_v1.csv` to the HAM
metadata shows what those images sit on:

| split | escalating images | escalating **lesions** | images/lesion | CP half-width at p=0.5, lesion level |
|---|---:|---:|---:|---:|
| test | 21 | **10** | 2.10 | **0.313** |
| val | 22 | **10** | 2.20 | **0.313** |
| OOF (train) | 64 | **34** | 1.88 | 0.176 |

Image counts reconcile with `results/age_band_prior.csv` exactly (`reconciles_with_split_file:
true` on all three), so the lesion counts are computed on the same cell, not a different one.
**Every published under-40 number in this repository is a proportion over ~10 independent units.**
`research/stats/intervals.py` already warned in its own docstring that Clopper-Pearson "ignores
lesion clustering, which makes it slightly anti-conservative when one lesion contributes several
images" — at 2.1 images per lesion in this exact cell, "slightly" was doing more work than it
should. The famous `3/21 = 0.143, [0.030, 0.363]` is the *optimistic* reading: the lesion-level
half-width is **0.221**, not 0.166.

This rescales V4's own justification. The counting unlock is not 21 → 150 images (~7x) and not the
~24x the runbook's §0 table implies. It is **10 lesions → 104+ lesions, a 10.4x gain** — a better
argument than the one the runbook made, resting on a smaller starting number.

**Seed-variance floor, from S44's accidental control.** Two retrains on byte-identical data gave
0.6364 and 0.4091: sample SD **σ = 0.1607 on one degree of freedom**. The χ² interval on that
estimate is `[0.0717, 5.13]` — the upper end is uninformative and is reported rather than hidden,
so the defensible claim is the **lower bound: σ ≥ 0.072**. The same-data spread (0.2273) exceeds
the between-condition spread (0.1818), confirmed computationally rather than restated.

**The seed policy the runbook asked for is not sufficient, and S48 says so.** The rule "≥ 3 seeds
for any condition whose effect lands within 2× the floor" has a threshold of **0.321**. The
declared MCID is 0.10 and every effect V4 plausibly produces sits far below that, so the rule is
unconditional in practice: **every V4 under-40 condition runs ≥ 3 seeds; a single-seed under-40
result is descriptive from here on.** But three seeds do not make an *unpaired* comparison
adequate — projecting σ to the reserved cohort asks for **~6 seeds per arm at n=151** (and ~41 at
n=22). V4 therefore declares the **paired, seed-matched** form as primary, where the
cohort-sampling term cancels and the requirement collapses to the discordant-case count. The three
seeds do two other jobs: a stability check on the point estimate, and the material for a real
multi-seed variance estimate that replaces this 1-df one as soon as S53 lands.

**Primary endpoint, declared and frozen.** `under40_escalation_sensitivity_at_fixed_referral_budget`
— escalating (mel/bcc/akiec/scc) lesions in patients under 40 that receive an escalating
prediction, on the S49 reserved cohort, at a global referral budget matched exactly to the frozen
V1 baseline's rate on the same cohort, unit of analysis the **lesion**, MCID **0.10**. Matching the
budget is what stops an arm buying sensitivity with referral volume. 0.05 was rejected as an MCID
because it sits inside the interval the data can produce — the same mistake V4 exists to stop.

**Retired endpoints.** Any proportion on ≤ 25 positive **lesions** is descriptive-only in V4: it may
be reported with its interval and may not gate a decision, license a mechanism claim, or serve as a
comparison baseline. Five quantities retire — both test figures (0.143 argmax and 0.238 under the λ
rule, 10 lesions each), the val 0.545, and **both S44 control retrains**. The OOF 0.547 survives on
34 lesions and *still* does not resolve the MCID (half-width 0.175): surviving the rule is not the
same as being adequately powered, and the report says so.

**The V4 falsifier: PREMISE HOLDS, with a correction the plan needs.** The single-arm exact interval
needs **104** under-40 escalating lesions to be narrower than the MCID; a paired McNemar comparison
needs **81** at a loss ratio of 0.5 (51 at 0.0, 121 at 1.0). The non-HAM pool supplies **151
lesions / 408 images** (BCN 115, MSKCC 36), and the SCC-excluded count of **150** reproduces S14's
published BCN 114 + MSKCC 36 exactly — an independent check that the `effective_lesion_id` logic
matches the one S14 used.

But the runbook's S49 gate — "≥ 150 under-40 escalating lesions reserved" — is written in lesions
while its supporting table counts images. Meeting it literally reserves **150 of 151 lesions** and
leaves **1** for training, contradicting §8's instruction to exhaust those cases first. **Action for
S49: size the reserved cohort at 104–131 under-40 escalating lesions, not 150, and record the
chosen split.** At 104 reserved, 47 under-40 escalating lesions remain trainable.

Artifacts: `research/v4/power.py` (`--out-dir`, `--no-ledger`), `results/v4/power_audit.json`,
`power_audit.csv` (36-row detectable-effect table), `historical_under40_audit.csv`,
`paired_power.csv`. Ledger: **4 rows** under `session=v4_s48`, prefix `S48_`; the runner prunes its
own prior rows (verified — a second run replaced 3 and left the diff at +4, no duplicates).

**Corrections applied on the finding (same session, at the owner's instruction).** S48's
lesion-count result is a correctness problem for the manuscript, not only a planning input, so it
was applied rather than filed.

1. **`paper/manuscript.tex` § Statistical protocol contained a factual error in the paragraph
   arguing the interval rule is not cosmetic.** It stated the principle correctly — "resampling
   images would treat correlated views as independent evidence and produce intervals that are too
   narrow" — and then described its own headline count as "$3$ of $21$ escalating **lesions**".
   They are images, on ten lesions. Fixed, and a new paragraph *A residual we state rather than
   resolve* now gives the per-split image/lesion pairs, states that the exact interval is computed
   on the image count and is therefore still too narrow, and explains why the lesion-denominator
   recomputation is **not** performed: it would compute a test quantity outside the frozen analysis
   plan. The published intervals are left untouched — renumbering them would require exactly that
   unregistered test read.
2. **A new Limitations paragraph, *Power: the paper's most cited number rests on ten independent
   units*.** Bounds the age analysis in two stated ways: nothing on this cell resolves a difference
   much below 0.2, so the λ rule's 0.143 → 0.238 sits inside the resolution and stays directional;
   and a difference that size is within same-data retraining spread, so no single run is evidence
   about this band. Names the fix as a data problem, which is what S49 does.
3. **§ Results-fairness** now says "escalating **images** caught" where it described image-level
   sensitivities as lesion-level.
4. **`research/ablation/audit_manuscript.py` 357 → 366 checks.** Five are new guards, and **all
   five were proved to fail** by reintroducing their defect
   (`scratchpad/prove_guards_fail.py`, the S19 pattern): the wording regression, the test/val
   lesion cells diverging, `power_audit.json` ceasing to reconcile against `age_band_prior.csv`,
   the cell ceasing to be clustered (which would make the new Limitations paragraph obsolete
   rather than merely wrong), and a test read appearing in the audit's own source. Structural
   validation passes; 49 cites = 49 bibitems.
5. **`V4_SESSION_RUNBOOK.md` S49 gate corrected ≥ 150 → 104–131 under-40 escalating lesions**, in
   the reserved-cohort paragraph and the session table, with the reasoning quoted inline. The
   original figure is unreachable: it takes 150 of 151 available lesions and leaves 1 to train on.

### S49 — The V4 corpus: the conventional dedupe threshold is noise on this data (2026-09-15)

**No test read.** Receipt stays at **`n_executions: 2`**; frozen checkpoints **6/6 byte-identical**.
Deliverables: `ml/data/manifest_v4.csv` (25,331 rows), `ml/configs/splits/v4/split_v4_{train,val,
reserved}.csv`, `results/v4/corpus_report.json`, `duplicate_clusters.csv`, `duplicate_stats.json`,
`dedupe_purity_sweep.csv`, `archive_probe_pre_post.{csv,json}`, `perceptual_hashes.npz`.
Modules: `research/v4/{build_corpus,dedupe,colour,archive_probe}.py`.

**Taxonomy moved to 8-class ISIC-2019** (MEL, NV, BCC, AK, BKL, DF, VASC, SCC), keeping the **628
SCC** images V3's pre-registered exclusion had to discard. Every row carries a **7-class collapse**
(AK+SCC → `akiec`) so any V4 number stays comparable to V1–V3. 0 UNK rows to drop — ISIC-2019's UNK
column is all-zero in the training set. `class_mapping.json`'s `planned_extension` block is the
authority for SCC being separate rather than folded into `akiec`.

#### The finding: the standard near-duplicate threshold is pure noise here

Near-duplicate detection had never been run in this project. The conventional rule — dHash **or**
pHash within Hamming 6 — produced **one connected component of 10,498 images, 38% of the corpus**,
fused by transitive chaining. `chance_rate()` explains why by measuring the background directly:

| rule | matches this share of **random** pairs | expected among the 3.8e8 pairs | observed |
|---|---:|---:|---:|
| dHash ≤ 6 | 0.0875% | ~334,000 | 360,363 |
| pHash ≤ 6 | 0.0015% | ~5,700 | 7,909 |
| either ≤ 6 | — | ~340,000 | 366,606 → **enrichment 1.08×** (0.95× at radius 3) |

*Corrected in the 2026-09-17 final audit.* The observed counts and the 10,498-image component
reproduce, but the first version of this table had no file behind it. Its chance rates (0.101%,
0.003%) and its 0.95× do not reproduce: 0.95× is the either-hash enrichment at radius **3**. The
recomputed figures are in `results/v4/audit/dedupe_conventional_rule.json` (`audit_v4 --emit`).
The conclusion is unchanged: either rule sits at ≈1× its chance rate.

**The entire conventional signal is the background rate.** Dermoscopy images are centred,
circularly-vignetted fields with one lesion in the middle, so their gradient structure is nearly
identical across thousands of unrelated nevi — dHash has run out of discriminating power and the
threshold cannot tell. The code was correct and the threshold is the textbook one; the result was
garbage.

Two corrections, both measured rather than assumed. **Require both hashes** (independent
false-positive rates multiply), and **choose the radius by label purity** — a true duplicate
cluster carries one diagnosis, so the share of multi-image clusters spanning more than one
diagnosis is a false-positive rate obtainable without knowing which images are really duplicates:

| radius | pairs | clusters>1 | images | largest | impure frac |
|---:|---:|---:|---:|---:|---:|
| 0–2 | 78–174 | 78–164 | 156–334 | 2–4 | 0.029–0.034 |
| **3** | **223** | **208** | **425** | **4** | **0.042** |
| 4 | 500 | 390 | 861 | 11 | 0.083 |
| 5 | 626 | 441 | 1,007 | 24 | 0.111 |
| 6 | 1,666 | 648 | 1,839 | **115** | 0.155 |

Impurity is flat through radius 3 then doubles while the largest cluster leaves the range a real
duplicate set can occupy — at radius 6 one cluster held 115 images across **102 `lesion_id`s and
eight diagnoses**. **S49 adopts radius 3, both hashes**: enrichment **≥1,005×** over a
conservatively bounded chance rate, against the conventional rule's 0.95×. The residual ~4%
impurity persists down to radius 0 (exact hash collisions with different labels) and is reported,
not tuned away. **0 cross-archive (ISIC↔PAD) clusters.**

`cluster()` now recomputes the chance rate every run and **refuses to group on a rule that is not
clearly above it**, so this cannot silently rot. The chance model is the product of the measured
marginals (the joint rate is far below any affordable sample's resolution), with the rule of three
bounding a marginal that returns zero hits — the assumption of hash independence off-duplicates is
named in the code rather than buried.

#### Grouping, splits, and the reserved cohort

`group_id` is the connected component of **lesion identity ∪ perceptual duplication**, not either
alone: **13,931 effective lesions → 13,909 groups, 22 merges forced by duplicates**. Those 22 are
exactly the leak `lesion_id` grouping alone would have missed. 2,084 null `lesion_id`s became
singletons (S14's pattern, reused). A group containing any HAM val/test image is assigned wholesale
to that split, so a non-HAM near-duplicate of a HAM test image *is* a test image.

Splits: **train 15,294 / val 2,270 / reserved 4,733 / ham_val 1,532 / ham_test 1,502**. All hard
checks pass — `assert_no_leakage()` on both the V4 splits and all splits, **0** groups spanning
splits, **0** duplicate clusters spanning splits, **0** HAM val/test rows in V4 train, HAM
assignments preserved byte-identically, 500/500 sampled paths resolve.

The reserved cohort holds **104 under-40 escalating lesions / 279 images**, hitting S48's target
exactly. That required allocating the `<40|escalating` stratum **by count rather than by share** —
a flat 30% yields only 45 — which is the trade S48 named: the endpoint is estimable only if the
cases carrying it are held out, and those are the cases the model would most like to train on. 47
under-40 escalating lesions remain trainable.

#### Colour constancy: a real effect that does not license the conclusion it invites

`research/v4/colour.py` implements shades-of-grey (Finlayson & Trezzi, Minkowski **p=6**),
per-image and parameter-free, closing the phantom rung §13 recorded as never implemented. The
archive probe was re-run before and after on the same instrument (`probe_archive` unchanged,
frozen `convnext_tiny_best.HAM-only.pt`):

| contrast | raw | + colour constancy | Δ |
|---|---:|---:|---:|
| HAM val vs **BCN-20000** (primary) | 0.9905 [0.9873, 0.9934] | **0.9518** [0.9431, 0.9604] | **−0.0387** |
| HAM val vs MSKCC | 0.9454 | 0.8683 | −0.0771 |
| all three | 0.9656 | 0.9126 | −0.0530 |

Intervals do not overlap: **illuminant statistics carry part of archive identity**, so V4's re-pool
is a different experiment rather than a re-run, and H4's failure gains a candidate mechanism. Two
things the verdict explicitly refuses to conclude: the residual AUC is **still 0.952**, so archive
identity is reduced and nowhere near removed — most of it survives colour normalisation and lives
in something else; and a mechanism for archive *separability* is not a mechanism for H4's
*failure*. The pooling experiment still has to be run and can still fail.

#### Two process findings

- **Batch size is part of the feature-extraction contract.** The raw arm reproduced S43's cached
  BCN/MSKCC features to **exactly 0.0** at `BATCH_SIZE=64` (S43's value) and to max 0.0235 at 32 —
  cuDNN selects convolution algorithms by input shape. Running at 64 flipped which cohorts were
  exact (ham_val's cache was made at a different size), which confirms the cause is batch size and
  not the code. The probe AUCs agreed to four decimals across both, so the conclusion is invariant
  to it; the check now reports max **and** mean deviation so a reader can tell a drift from a
  perturbation.
- **The no-prune ledger hazard fired a fourth time.** `archive_probe.py` had no prune and
  duplicated its six rows; worse, `build_corpus.py`'s prune matched the session-wide `S49_` prefix
  and would have deleted them. Both now use runner-specific prefixes (`S49_corpus`,
  `S49_archive_probe`) and prune only their own. Ledger verified: **11 v4 rows (4 S48 + 7 S49), 0
  duplicate (session, method) pairs.**

### S50 — The age shortcut IS peri-lesional, and the effect is too small to act on (2026-09-15)

**No test read**; GPU 0, as planned — the features were already on disk. Deliverables:
`research/v4/localise_shortcut.py`, `research/v4/mask_augment.py`,
`results/v4/shortcut_locus.{json,csv}`, `results/v4/shortcut_locus_bleed.csv`.
Ledger: **2 rows** under `v4_s50_locus`, prefix `S50_locus`, prune-own-only. Verified **13 v4
rows (4 S48 + 7 S49 + 2 S50), 0 duplicate (session, method) pairs.**

#### The verdict: directionally confirmed, quantitatively insufficient

S42's `age_band` (0.6922) and `lesion_vs_context` (interior 0.8714 / exterior 0.8608) had never
been composed. Running S42's **own** probes — imported from `research/v3/probes.py`, not
reimplemented — separately on the 6,965 × 768 interior and exterior vectors:

| probe | interior | exterior | paired Δ (ext − int) | |
|---|---:|---:|---:|---|
| `age_band` (macro-OvR AUC) | 0.6695 | 0.6991 | **+0.0297 [+0.0174, +0.0424]** | CI excludes 0 |
| `age_residual` (strat. Spearman) | +0.1518 | +0.1536 | +0.0018 [−0.0312, +0.0358] | null |
| `escalation` (AUC) | 0.8744 | 0.8709 | −0.0035 [−0.0172, +0.0110] | null |
| **selectivity** (Δage − Δesc) | — | — | **+0.0332 [+0.0147, +0.0522]** | CI excludes 0 |

**Every sign points the way the hypothesis predicted and the magnitude does not clear the bar.**
Age *is* encoded more strongly outside the lesion, and peri-lesional skin *does* carry age
preferentially over the diagnosis — both with intervals excluding zero. But the primary Δ is
**+0.0297 against a pre-declared MCID of 0.05**, and the selectivity margin's *upper* bound
(+0.0522) only just reaches it. **Verdict `CO_LOCATED`; R3 is DEMOTED on magnitude, not on
direction.** The runbook's binary framing — "exterior ≫ interior" or "interior ≈ exterior" — did
not anticipate this third outcome, and writing it up as a flat null would misreport it.

The decision rule, the MCID and the three-way verdict were written into the module and into
`declared_before_running` in the JSON **before** the probes were run. 0.05 is the scale S51
declares for its own backbone falsifier.

#### Why the contrast is paired, and what that bought

Two per-arm intervals are not a test of their difference, and the arms share every row, fold and
lesion. `paired_delta` resamples lesions once and evaluates both arms on the same draw, so the
sampling cancels out of the difference. Self-test check 4 measures the gain on its **synthetic**
fixture: the paired interval is **0.0028 wide against ≥0.0131** for a differenced-unpaired
reading, **4.7× tighter**. These are self-test figures with no `results/` file. On the real data
the paired `age_band` interval is 0.025 wide. Check 5
asserts rather than assumes that `GroupKFold` hands both arms identical folds.

#### Three controls, and one of them refutes the obvious objection

- **Receptive-field bleed.** A 7×7 ConvNeXt cell sees far more than its 32×32 patch, so the
  exterior pool is not lesion-free — bleed shrinks |Δ| toward zero, which is why a `CO_LOCATED`
  reading is a statement about the instrument, not about skin. `control_bleed` puts a number on
  it: if exterior signal were bleed, it would have to **strengthen** as the lesion grows to fill
  more of each exterior cell's receptive field. Exterior age AUC by lesion-area tertile is
  0.6926 / 0.6977 / 0.6973, large-minus-small **+0.0047 [−0.0207, +0.0301]** — **flat**. Bleed
  cannot produce a flat profile. The exterior block is reading skin, not an echo of the lesion.
- **Geometry.** Lesion **area alone** predicts age band at **0.5483 [0.5368, 0.5599]** — above
  chance. Part of what both blocks read is lesion *size*, which is available to both pools and
  removable by no mask. A confound the interior/exterior framing cannot see.
- **Reproduction.** `lesion_vs_context` reproduces S42 to **2.7e-05 / 3.6e-05** before anything
  downstream is trusted.

#### The finding that would have flipped the gate: 38 rows, 0.55% of the corpus

S42's `age_band` took `np.unique(bands)`, so its **38 `age_band='unknown'` rows entered as a
fourth class** — and macro-OvR gives a 38-positive class a **full 25% of the headline number**.
Dropping them is the pre-registered primary. Keeping them gives Δ **+0.0507 → verdict
`PERI_LESIONAL`**, i.e. **the opposite decision**, and R3 would have been promoted to a GPU
training run on the strength of it.

The per-class breakdown says exactly where that comes from: exterior predicts `unknown` at
**0.797** against interior's 0.675, a Δ of +0.122 that is **60% of the entire +0.0507** while
resting on 0.55% of the rows. "Age unknown" is not an age band — it is a record-keeping property,
and it is plausibly an *acquisition* read, which is the signal S49 just showed partly lives in
illuminant statistics. Both variants are in the JSON; the primary drops them and says so.

#### Post-hoc, no interval, no family: the under-40 band is where the effect lives

Decomposing the pre-registered 3-class fit per band — **point estimates only, not a declared
test, no CI, in no multiplicity family**:

| band | interior | exterior | Δ |
|---|---:|---:|---:|
| 40-59 | 0.608 | 0.619 | +0.011 |
| 60+ | 0.718 | 0.743 | +0.025 |
| **<40** | 0.683 | **0.735** | **+0.053** |

The band carrying the project's entire endpoint is the one band whose exterior advantage reaches
the MCID. This does not change the verdict — the primary is the macro average and it was declared
— but it is the sharpest reason to re-test the locus on a better representation in S51 rather
than treat R3 as closed. **If S51 pre-registers this, it must be declared before extraction.**

#### `mask_augment.py`: built, unwired, and its own self-test found the bug

`LesionCentredCrop` and `ExteriorDropout` are built as the runbook asks, imported by no training
path. The real-mask check the runbook required is what earned its keep: against 1,000 Tschandl
masks the first version **silently trimmed the lesion on 76 of 399** — HAM frames are 600×450 and
the mean lesion already fills 0.42 of the centre crop, so a padded square around a large lesion
routinely does not fit. Margin is now negotiable and containment is not; too-large lesions are
**declined** rather than cut. Final: **996/1000 usable, 0 lesion pixels lost, 0 errors**. (The
figures in this subsection are `mask_augment.py --selftest` output; no `results/` file holds them.)

That guard produced a second number worth keeping: the crop **declines 18.9% of usable lesions**
as too large to frame. R3's crop would be a no-op on nearly a fifth of the corpus — an
independent limit on the lever, arrived at from geometry rather than from the probe.

`DEFAULT_P = 0.25` is deliberately timid and untuned: exterior escalation AUC is 0.8709 against
the interior's 0.8744, so peri-lesional skin carries nearly as much diagnosis as the lesion does,
and the +0.0332 selectivity margin is too thin to license an aggressive drop rate.

#### Two defects in this session's own instrument, both caught before the writeup

- `control_bleed` set a boolean from a **+0.0047 point estimate with no interval** — noise
  deciding a control, the exact mistake the control exists to prevent. It now bootstraps
  large-minus-small on shared lesion draws; self-test check 9 proves it stays flat on
  area-independent signal.
- The **selectivity margin was reported bare**, despite being the declared quantity that decides
  whether R3 is worth a training run. It now carries a fully paired interval (check 8 proves it
  separates age-outside/diagnosis-inside from both-outside).

### S51 — A derm foundation model does NOT beat the in-repo trunk under 40 (2026-09-16)

**No test read.** GPU: three frozen forward passes over the 4,733-image reserved cohort,
**~2 minutes total**, not the 1–2 h the runbook budgeted — the corpus is small and nothing is
trained. Deliverables: `research/v4/extract_backbone_features.py`,
`research/v4/backbone_probe.py`, `research/v4/features/{convnext_tiny,panderm_vitb16,
dinov2_vitb14}_reserved.npz` (+ `.meta.json`), `results/v4/backbone_probe_plan.json`,
`results/v4/backbone_probe.{json}`, `results/v4/backbone_probe_{marginals,deltas}.csv`.
Ledger: **4 rows** under `v4_s51`, prune-own-only, verified idempotent across two consecutive
runs (419 ledger rows both times, 0 duplicate `(session, method)` pairs).

#### The verdict: the pre-registered falsifier FIRED

| contrast, `<40`, primary pooling | Δ pAUC@0.20 | lesion-grouped paired CI | verdict |
|---|---:|---|---|
| **`panderm_vitb16` − `convnext_tiny`** | **−0.0363** | **[−0.0828, +0.0127]** | **NOT_CERTIFIED** |
| `dinov2_vitb14` − `convnext_tiny` | −0.0725 | [−0.1180, −0.0288] | NOT_CERTIFIED |
| `dinov2_vitb14` − `panderm_vitb16` | −0.0362 | [−0.0779, +0.0034] | NOT_CERTIFIED |

The falsifier required PanDerm to **exceed** ConvNeXt-Tiny by ≥ 0.05 with a CI excluding zero.
It is **negative**, and the interval crosses zero. Neither condition is met, and not narrowly:
the whole interval sits below the MCID. **`REPRESENTATION_BOTTLENECK_FALSIFIED` — V4 drops the
foundation-model arm; the recipe ladder proceeds on the in-repo trunk.** This is the cheap
verdict the plan was built to buy, and it came back in the direction that costs nothing.

Per-backbone under-40 pAUC (adaptive supremum over the S41 probe family, cross-fitted on 104
under-40 escalating lesions / 279 images): `convnext_tiny` **0.7319 [0.6597, 0.7979]**,
`panderm_vitb16` **0.6956 [0.6251, 0.7688]**, `dinov2_vitb14` **0.6593 [0.5924, 0.7312]**
(`results/v4/backbone_probe_marginals.csv`).

#### Four things the secondary readouts say, none of which rescue the arm

1. **PanDerm is not a weak representation — it is weak *where it matters here*.** It **beats**
   the control at 60+ (+0.0382 [−0.0012, +0.0754]) and all-ages (+0.0138 [−0.0065, +0.0358]),
   and loses only in `<40`. Neither direction is certified, so the honest statement is that the
   two representations are indistinguishable overall and the sign flips across age bands.
2. **The two foundation models reproduce the under-40 deficit the control does not.** Within
   backbone, `panderm` runs 0.6956 → 0.7301 → 0.7582 and `dinov2` 0.6593 → 0.7025 → 0.7084 as
   age rises, while `convnext_tiny` is 0.7319 → 0.7381 → 0.7200 — under-40 is its *middle* band,
   consistent with S14's finding that "under-40 is the worst-ranked band" does not replicate on
   BCN/MSKCC. The intervals overlap heavily; this is a pattern in the point estimates, not a
   certified ordering, and must not be quoted as one.
3. **DINOv2 loses to the control in `<40` with the interval excluding zero** (−0.0725
   [−0.1180, −0.0288]). A general-purpose web-pretrained representation is measurably worse than
   a small supervised derm trunk on this endpoint. That is the control the plan asked for, and
   it behaved as a control should.
4. **The verdict survives the pooling ambiguity.** DermLIP's config does not state whether its
   head consumed CLS or mean-pooled patch tokens, so both were extracted in the same pass and
   the whole battery was run twice. Under mean-pooling the headline contrast is **−0.0077
   [−0.0489, +0.0364]** — still negative, still crossing zero, still far below +0.05. No pooling
   choice turns this into a pass.

Every backbone's adaptive probe selected `gbm` or `mlp` in every outer fold and **never**
`linear` or `multinomial_lse`, re-confirming S41's point that a linear probe is the wrong anchor
for a log-sum-exp readout.

#### The licence decision, which the runbook required before S53

**It does not arise, and that is the best available outcome.** PanDerm is **CC-BY-NC-4.0** —
academic use only, product-blocking. The arm is dropped **on evidence**, not on licensing, so
V4 proceeds on the in-repo ConvNeXt trunk and the deployed product stays free of a
non-commercial dependency. Had the falsifier cleared, the project would have faced a real
choice between the manuscript and a shippable product; it does not.

#### Two deviations and one defect, all recorded

- **D1 — DINOv2 pinned to 224 px.** timm's default config for `vit_base_patch14_dinov2.lvd142m`
  is **518 px**. Running it there would have handed that arm a 5.3× token budget over two 224 px
  arms and confounded the representation question with input resolution, which is rung **R1**'s
  separate question in S52. It also exhausted host RAM. Pinned to 224 (the standard DINOv2
  linear-probe setting) so the only thing varying between arms is the representation.
- **D2 — the PanDerm checkpoint is DermLIP's CLIP-aligned PanDerm-base, not the raw
  masked-latent release.** The official weights ship as pickled `.pth` on Google Drive; the only
  HF copies are unaffiliated third-party mirrors with no licence and zero downloads, and loading
  one means unpickling code from an untrusted host. `redlessone/DermLIP_PanDerm-base-w-PubMed-256`
  is the authors' own account, ships **safetensors**, and declares exactly the tower the plan
  names (ViT-B/16, 224 px, 12 layers, width 768). The 188-key BEiT→timm remap is asserted
  exhaustive and proved non-trivial by a self-test that compares the loaded tower against a
  random init (max|Δ| = 10.01).
- **Defect, caught and fixed: the declared robustness arm silently dropped the control.**
  `load_features` returned `None` when a backbone had no `features_alt`, and ConvNeXt has none —
  so the first run's robustness arm contained only the two ViTs and could not evaluate the
  pre-registered contrast at all. It now falls back to the control's single representation and
  records `pooling_used` per row. The numbers in item 4 above come from the fixed run.
- **Defect, cosmetic: `--freeze-plan` printed the wrong digest.** It hashed the in-memory string
  while Windows wrote CRLF to disk, so the printed `844aab69…` could not be reproduced from the
  artefact. `plan_sha256()` always read the file, so the digest recorded inside
  `backbone_probe.json` — **`09d5795ef02eca14…`, the authoritative one** — was correct
  throughout and no result moves. The printout is fixed and newlines are now pinned.

#### The bias that makes this negative result strong

PanDerm's 2M-image derm corpus and DINOv2's LVD-142M are undisclosed at the image level and may
well contain ISIC-2019 material, so both candidates could be partly **in-distribution** on a
BCN/MSKCC cohort. The control cannot be: `convnext_tiny_best.HAM-only.pt` was fitted on HAM
train, and the reserved split contains **zero** HAM images — asserted, not assumed
(`extract_backbone_features.py --selftest`, check 2: overlap = 0). The control is further
disadvantaged by being the only supervised, single-domain model in the comparison. Every one of
those biases runs **toward** the alternative hypothesis, which is exactly why a negative is
robust here and a positive would have needed the caveat attached.

### S52 — The recipe ladder: seven arms pre-registered, and the S54 gate was confounded (2026-09-16)

**No test read. No GPU.** S52 builds and freezes; S53 is the user's overnight training block and
no interactive session runs during it. Deliverables: `research/v4/recipe.py` (the frozen rung registry
plus every component the arms switch on), `research/v4/train_v4.py` (the runner, one arm per
invocation), `results/v4/recipe_ladder_plan.json` — **sha256
`1fb913413545b8e5e63d38c437e9149b1874557a181815dd58100fc363e68954`**, hashed from the bytes on
disk with newlines pinned, so the S51 CRLF printout bug cannot recur. Ledger: **1 row** under
`v4_s52`, prune-own-only, verified idempotent across two consecutive freezes (420 rows both
times, digest unchanged). `recipe.py --selftest` runs **37 checks, all passing**.

#### The arms

| Rung | Change | Declared as | Endpoint | MCID |
|---|---|---|---|---:|
| **R0** | none — the published recipe verbatim | reproduction anchor | val Macro-F1 in **[0.735, 0.765]** | — |
| **R1** | 224 → **384 px** | ranking lever | Δ val Macro-F1 | 0.020 |
| **R2** | shades-of-grey colour constancy | ranking lever | Δ val Macro-F1 | 0.020 |
| **R3** | mask-guided crop | **DROPPED** — S50 gate fired | — | — |
| **R4** | class-balanced sampler | **operating-point lever** | Δ escalation sens. at matched referral | 0.05 |
| **R5** | Mixup/CutMix + colour-safe RandAugment | ranking lever | Δ val Macro-F1 | 0.020 |
| **R6** | EMA + 60-epoch cosine | ranking lever | Δ val Macro-F1 | 0.020 |
| **R7** | explicit metadata branch (age/sex/site) | **mechanism** | counterfactual age-flip shift in escalation mass, + non-inferiority on Macro-F1 | 0.05 |

Only **R1/R2/R5/R6** are ranked by the screen. R4 and R7 are exempt **by prior declaration**, which
is the point of declaring them before they run: S36's `verify_losses.py` check 7 already proves a
band-constant offset cannot change within-band ranking, so a null Macro-F1 result for a balanced
sampler is *predicted*, not a failure — and R7 is a mechanism whose claim is auditability, which
cannot be ranked against a lever. R3 is dropped with its reason carried in the registry, and
`train_v4.py --rungs R3` refuses to run and prints it.

#### The screen selects on Macro-F1, never on an under-40 quantity — and that is measured, not stylistic

This is S52's load-bearing decision. S44's control pair retrained one identical condition twice
(`results/v3/s44_control_and_multiplicity.json`):

| endpoint | same-data seed spread | between-condition spread | |
|---|---:|---:|---|
| val Macro-F1 | **0.0027** | 0.0447 | signal ≈ **16×** noise |
| under-40 escalation sensitivity | **0.2273** | 0.1818 | **noise exceeds signal** |

S48 turned the second row into `seed_floor = 0.1607` and the rule that every V4 under-40 condition
needs ≥ 3 seeds. Screening seven arms on a 22-positive endpoint whose seed noise exceeds its
between-condition signal would be **selecting on noise**, and would bias the S54 readout that the
reserved cohort was built to support. The under-40 endpoint is therefore read **once**, at S54, on
the reserved cohort's 104 escalating lesions. The screen is declared in the plan as a
**compute-allocation filter, not a hypothesis test** — no p-values at Block 1, and "screened out"
never means "falsified".

The MCID of **0.020** on val Macro-F1 is anchored, not chosen: it is ~7× S44's measured seed spread
of 0.0027, and it is the size of the V1 Dirichlet rung A6→A7 (**+0.019**) and larger than the TTA
rung A5→A6 (**+0.014**) — both reported, load-bearing levers in the manuscript, so an effect below
it is not one the paper would describe. The non-inferiority margin of **0.005** is ~2× the same
spread. The operating-point and age-flip MCIDs of **0.05** are anchored on the frozen under-40
λ of 0.26, which moved test under-40 sensitivity 0.143 → 0.238.

#### ⚠️ The S54 gate as the runbook worded it is confounded, and would have licensed a false claim

The runbook's gate is "the best V4 representation must beat **the frozen V1 ensemble** on the
reserved cohort". Verified against `ml/data/manifest_v4.csv`: the reserved split is **4,733 images,
0 of them HAM** (3,856 BCN-20000 + 877 MSKCC), and V4's pooled corpus trains on those archives
while the V1 ensemble never did. S14 already measured V1's BCN transfer at Macro-F1 **0.402**
against **0.784** in-domain. That gate is therefore expected to fire on Macro-F1 **for
training-corpus reasons alone**, and reading the runbook's four outcomes off it would license
"the representation was the bottleneck" from what is really a training-data change.

The plan splits it in two, recorded as a **declared deviation**:

- **Gate A** — composite vs frozen V1 ensemble. Kept as the runbook declares it, labelled
  **confounded**, and reported as the *deployment* delta only.
- **Gate B** — composite vs the **V4 pooled control**: same corpus, same split, seed-matched
  paired. The clean recipe contrast, and **the four outcomes are read from this one**.

Both endpoints must move with a lesion-grouped CI excluding zero. The conjunction is conservative
by construction, so no multiplicity correction is applied across the two.

#### Two facts found while building, both of which change what S54 has to do

1. **The V1 comparator is already frozen — no new inference pass is needed for 97% of it.**
   **4,587 of the 4,733** reserved images already carry 6-CNN soft-vote + 24-view TTA + deployed
   Dirichlet predictions in `results/external/predictions/` from S13/S14. Same precedent as S10's
   `_render_table_only` and S12's `s9_test_anchor()`: read the frozen artifact, do not re-read.
2. **⚠️ The 146 missing images are exactly the `scc` rows S13 excluded by pre-registration, and
   dropping them costs a lesson.** Without them the cohort falls to **103** under-40 escalating
   lesions — **one below S48's power target of 104**, and no longer the same cohort S51 read its
   0.7319 / 0.6956 / 0.6593 pAUCs on. S54 must score those 146 images with the six frozen
   checkpoints (minutes of GPU, inside S54's existing inference budget) so both arms are read on
   one 4,733-image cohort. Asserted in `recipe.py --selftest` checks 4.a–4.c.

#### Three more things declared now so they are not discovered later

- **V4 `val` contains zero HAM images.** The Block 1 screen model-selects on a HAM val
  distribution and Block 2 on a BCN/MSKCC one, so a rung can reverse between them. A declared
  weakness of the screen → promote design, not a finding.
- **Block 3 costs three nights, not one, if R1 is promoted.** S48's binding rule applies
  unconditionally to both Block 2 arms. At 384 px that is ~5.3 h × 4 additional runs ≈ **21 h**;
  at 224 px it is ~1.8 h × 4 ≈ 7 h. The runbook's one-night estimate is correct only conditional
  on R1 failing. Paired, seed-matched is primary (S48's own resolution); report the seed-averaged
  estimate with its range and **never the best seed** (S44 is the precedent — either of its two
  retrains would have supported an opposite conclusion).
- **The composite, not the individual rungs, is promoted.** V1's exhaustion result is five levers
  each below noise, so composition is the hypothesis worth the GPU time. The cost is stated in the
  plan: an effect on the composite cannot be attributed to one rung.

#### Build notes

`ml/training/train.py` is **not** touched — it is the provenance of the six published baselines,
it reads a manifest column the V4 corpus does not have, and it has no CLI route to image size,
seed, sampler, EMA, mixup or a two-input model. S51's extractor hit the same wall and made the
same call. The recipe is reachable **only** by composing registry entries (`--rungs R1 R5`), so
no parameter can be typed at the command line — the `frozen_params.py` discipline from S12,
applied to training. `write_ledger` prunes its own prior rows, the hazard S20 recorded twice.
R7 needed a **new** tabular encoder: `research/fusion/tabular.py` carries HAM's localisation
vocabulary while the V4 manifest carries ISIC-2019's, and sharing it would have silently one-hot
encoded every V4 row into `unknown` (asserted, check 6.d).

**Proved, not assumed:** all seven arms plus a four-rung composite were smoke-run end to end on
CPU, including the checkpoint save and run-JSON paths — `--smoke` deliberately *writes* its
artefacts to `results/v4/recipe_runs/smoke/` rather than skipping them, because a rehearsal that
skips the save path does not prove the thing that costs a night to discover. Checkpoints carry the
fitted tabular-encoder state and the plan hash, so S54 can reproduce R7's features exactly. The
composite-conflict guard, the R3 refusal and the `ham_test` refusal were each proved to fire by
triggering them.

#### Block 0 was run, and it changed three things the plan had asserted

The GPU rehearsal is normally S53's first step. It was run inside S52 instead — 5 minutes, writes
nothing of record — because it validates code written in this session, and it found three defects
that a Block 1 overnight would have paid for.

1. **⚠️ 4 dataloader workers die at 384 px on the pooled corpus.** Windows error **1455**
   (`ERROR_COMMITMENT_LIMIT`) — not VRAM. This host has a 30.4 GB commit limit against 22.9 GB
   already committed, ~**7.5 GB headroom**, and each Windows worker is a full spawned torch
   import. 0/2/3 workers pass, 4 fails. **The default is now 2**, which is free where it matters:
   at 384 px, 2 and 3 workers are indistinguishable (**306 vs 307 ms/batch**) because the GPU is
   the bottleneck. This failure sits exactly in the Block 2 configuration that runs for hours.
2. **⚠️ Batch 16 at 384 px would have confounded R1.** The runbook expected 384 px to need batch
   16 on 8.5 GB; measured, it peaks at **1.75 GB of 8.55 GB at batch 32**. Keeping 16 would have
   made R1 differ from the control in **two** factors — resolution *and* effective LR schedule —
   while the plan declares it as one, and the screen would have attributed a batch-size effect to
   resolution. **Batch is now 32 across the whole ladder.** VRAM peak across all seven arms:
   1.64–1.89 GB.
   ⚠️ *Final-audit correction (2026-09-17):* these peaks were taken with the backbone frozen, not
   in the fine-tune stage. The banked runs peak at 2.41 GB at 224 px and 3.49 GB at 384 px
   (batch 16 × accum 2), per `recipe_runs/*.json`. The measured fine-tune anchor at 384 px,
   batch 32 is 6.27 GB, and the composite used ~6.2 GB when launched that way (S53). R1 and the
   composite in fact ran at 16 × 2, which is also effective batch 32. The single-factor argument
   survives; "384 px fits easily at batch 32" does not.
3. **The runbook's "~50 min per rung" is 3.4× too slow.** Measured at batch 32 / 2 workers:
   30 epochs over 6,981 images at 224 px is **14.8 min**; 384 px costs **2.26×**, not the 2.94×
   a pixel-square model predicts. **The whole three-block programme is ~9–13 h, not ~40 h**, so
   the "three nights versus one" contingency the plan previously carried **does not arise**.

A fourth measurement reverses the ladder's apparent shape: **R2, not R1, is the expensive screen
arm.** `shades_of_grey` costs **18.8 ms/image** against the control's 3.5, because S49 specified
float64 for the power-6 term (float32 loses the low bits that separate two similar illuminants).
That is a deliberate, frozen preprocessing choice and is not optimised away; R2 is simply
data-bound at 224 px and costs what R1 does. The estimator now takes `max(GPU, input pipeline)`
per arm rather than assuming the GPU always leads.

**The plan was re-frozen** to carry the measured timings:
`1fb913413545b8e5…` → **`eff9d79f36f351ff…`**. Legitimate because no data had been collected —
and checkable: a payload diff shows **exactly one key changed**, `/seeds/budget_contingency`, with
**no MCID, endpoint, falsifier, gate or recipe field touched**.

The S53 command blocks are **generated from the registry** — `recipe.py --commands`, and
`--commands --promote R1 R5` once Block 1 is read — rather than written into a handoff document
that would drift from the arms it describes. A **Block 0** GPU rehearsal is emitted ahead of
Block 1: it runs all seven arms at their declared batch sizes in ~5 min, because an OOM there
costs minutes and the same OOM at 03:00 in Block 1 costs the night. Batch sizes (32 at 224 px,
16 at 384 px) are declared, not verified — Block 0 is what verifies them, and every run writes
its achieved batch size to the ledger. *(Superseded by point 2 above: the plan's batch is 32
everywhere. At 384 px it is run as 16 × grad-accum 2.)*

### S52 Block 1 readout — the control gate fails on noise, not on the trainer (2026-09-16)

No test read, no GPU. The three unattended scheduled tasks (`v3-s44-s45-unattended`,
`v4-morning-diagnostic`, `v4-morning-readout`) were deleted at the owner's request; the morning
diagnostic never produced `results/v4/morning_run.log`, so neither the R1 re-run nor the control
reference ran. `research.v4.screen` was re-run over the six arms on disk
(`results/v4/screen_verdict.json`).

Block 1, seed 42, HAM-only, best val Macro-F1 (`results/v4/recipe_runs/*_ham_only_s42.json`):
R0 **0.7773**, R4 0.7682, R7 0.7663, R6 0.7651, R5 0.7482, R2 **0.6751**. **R1 is missing** — the
overnight attempt died at epoch 3 (error 1455); its `convnext_tiny-v4_R1_*` checkpoints are that
dead run's, not a result. The gate still fails (0.7773 vs band 0.735–0.765), so Block 2 has not run.

**Diagnosis.** `train_v4.py` and `ml/training/train.py` share every declared hyperparameter
(3 head epochs, LR 1e-3/1e-4, WD 1e-4, label smoothing 0.05, effective-number weights, cosine,
patience 8, `drop_last`, selection on val Macro-F1) and the same transforms. What differs is the
statistic, not the trainer:

- R0's 0.7773 is a **single-epoch spike** (epoch 15; neighbours 0.7515 and 0.7473). Its fine-tune
  median is **0.7469** — inside the band and ~0.001 from the published 0.7482. Mean epoch-to-epoch
  change 0.0216.
- The reference trainer is just as jumpy: `convnext_tiny-v3_ham_only` (S44) mean |Δepoch| 0.0140,
  fine-tune SD 0.0235; the untagged `convnext_tiny` history 0.0263 / 0.0301 (SDs over all
  fine-tune epochs, ddof 1; the first version quoted 0.0211 / 0.0216, which the final audit could
  not reproduce under any stated definition)
  (`ml/results/*_training_history.json`). Those two reference runs' fine-tune medians differ by
  **0.023** (0.7290 vs 0.7055) — larger than R0's 0.018 lift over S44 — though the untagged
  history's provenance is unknowable (bug A.0.2), so this is supporting, not decisive.
- ⚠️ **The 0.0027 "same-data seed spread" is one pair**, |0.7509 − 0.7482|. The control band
  (±5×), the MCID's "~7× seed spread" anchor and the 0.005 non-inferiority margin all rest on n=2,
  applied to a max-over-epochs statistic whose within-run jitter is ~0.02. The band is too narrow
  for the quantity it gates. This is a finding about the plan; it does **not** license moving the
  band after seeing R0 — the pre-registered resolution (reference trainer on the same split,
  agreement within the MCID) still stands.

**Consequences for the screen**, independent of the gate:
- No ranking lever clears the MCID; R2/R5/R6 are all below the control, so only R1 could be
  promoted. R2 (−0.102) is the only delta clearly outside run-to-run noise.
- **R4 clears its declared endpoint** (final-epoch escalation sensitivity 0.7987 vs 0.7370,
  +0.062 ≥ 0.05), so the composite is at least `[R4]`. ⚠️ R0's escalation sensitivity swings
  0.659–0.792 across its last six epochs and the comparison is at unmatched referral, so this is
  a declared-rule pass, not evidence R4 works.
- **R7 misses its mechanism endpoint by 0.0025** (age-flip span 0.0475 vs 0.05), and at
  −0.011 it is outside the 0.005 non-inferiority margin too — though that margin rests on the
  same n=2 spread, so at a single seed it cannot be met by anything short of a tie.

Open: R1 re-run and the control reference (`scripts/run_morning.ps1`, owner-launched).

### S53 — Block 1 closed, the control gate resolved, Block 2 running (2026-09-16, afternoon)

**Supersedes two statements in the readout above**, which was written before either run existed:
R1 is no longer missing, and the control reference has run. No test read.

**R1** re-ran at 384 px with batch 16 × grad-accum 2 (effective batch 32, unchanged from every other
arm) on one worker: best val Macro-F1 **0.7998** at epoch 25 of 30
(`results/v4/recipe_runs/R1_ham_only_s42.json`) — **+0.0225** over R0, **clearing the 0.020 MCID**.
Accumulation reproduces batch 32 exactly for an unweighted loss and to ~5e-04 on the weights for the
class-weighted loss used here (a seed-level wobble, measured and declared in `train_one_epoch`).
The script was launched three times within ten minutes (13:20, 13:23, 13:30), so up to three R1
copies wrote the same checkpoint names; the surviving `R1_ham_only_s42_best.pt` was checked and
matches the run JSON exactly (epoch 25, 0.7998), so the result is intact. A named mutex now makes a
second concurrent launch exit immediately.

**The control gate, resolved by the pre-registered rule.** `ml/training/train.py` on the identical
split scored **0.7765** at epoch 25 (`results/v4/control_reference.json`), **0.0008** from
`train_v4`'s 0.7773 — well inside the 0.02 agreement tolerance. The two trainers behave the same;
the band was set against a stale reference, not a broken pipeline. `screen_verdict.json` records
`FAILED_BUT_DEVIATION_ACCEPTED`. This **corroborates the diagnosis above** — the published 0.7482 and
S44's 0.7509 were single draws of a max-over-epochs statistic with ~0.02 within-run jitter, and the
band built on their n=2 spread was too narrow for the quantity it gates. The band itself was not
moved; the declared rule decided.

**The screen.** Only R1 clears the MCID; R6 −0.0121, R5 −0.0290 and R2 −0.1022 are all below the
control, so the second Block 2 slot is **unfilled** rather than given to a negative lever. R4 meets
its own endpoint (escalation sensitivity **+0.0617**, at unmatched referral) and joins; R7's age-flip
span of **0.0475** misses 0.05. **Composite: R1 + R4.** Block 2 started 14:32 — pooled control
(R0, 224 px) then composite (384 px), projected to finish ~16:55–17:40.

**Defects found and fixed while closing this out:**
- `screen.py` labelled every promoted rung "below MCID" whenever fewer than two cleared,
  misreporting R1; the basis is now recorded per rung.
- The Block 2 loop launched every arm at batch 32 / 2 workers — for a 384 px composite, the exact
  configuration that crashed R1. 384 px arms now get the batch-16 × accum-2, 1-worker
  accommodation, and arms with a banked result are skipped so a relaunch resumes. The instance
  already running predates this fix; it has 10 GB of disk behind it, the condition whose absence
  caused the overnight crash.
- The Block 2 projection priced the control at 384 px; the control is always R0 at 224 px.
- `train_v4` now refuses to start below 5 GB free and stops cleanly below 2 GB. The overnight
  failure surfaced as Windows error 1455 and was misdiagnosed three times (VRAM, checkpoint write,
  then commit limit) before the chain was found: **full disk → auto-managed pagefile cannot grow →
  commit limit pinned → dataloader cannot map shared memory.** Freeing disk raised the commit limit
  33.7 → 37.8 GB on its own.
- R1's ledger note recorded `batch_size=16` without the accumulation; it now reads
  `grad_accum=2 effective_batch=32`, rewritten from the run JSON.
...
- The Block 2 pooled control banked: **0.5768** at epoch 7, early-stopped at 15, 25.3 min
  (`R0_pooled_s42.json`). Pooled val is 100% BCN/MSKCC, so it is not comparable with HAM val.
- **Every job launched from the desktop tool runs inside the desktop app's MSIX package container** —
  on the local GPU, but tied to the app's lifetime. Verified: a WMI-created process does not see the
  virtualised container AppData, and is parented to WmiPrvSE. The composite (then ~10 min in, at
  batch 32 / 2 workers, ~6.2 GB VRAM against 1.2 GB free RAM) and the waiting Block 3 were stopped
  and relaunched via WMI. The composite restarted at 15:06 on batch 16 × accum 2, 1 worker
  (~4.5–5 min/epoch, data-bound).
- A Block 3 launch had silently failed: PowerShell 5.1 `Start-Process -ArgumentList @(...)` does
  not quote items, so the spaced repo path split.
- `scripts/run_block3.ps1` rewritten to be self-healing: it covers all six pooled runs, including
  seed 42's composite if Block 2 never banks it. It retries with workers 2 → 1 → 0, kills runs whose
  checkpoint stalls 30 min (including a hung lock-holder), has a disk floor, validates each run
  JSON, continues past a permanent failure and writes a per-minute `block3_status.json`. All paths
  were exercised in a sandbox with a fake trainer. `train_v4` now records `num_workers`, since
  retries may change it.

### S54 build — the reserved-cohort gate, pre-registered and rehearsed; no reserved read (2026-09-16, evening)

**Block 3 finished on its own at 19:34; nothing in this session touched it.** All six pooled runs
are banked (`results/v4/recipe_runs/{R0,R1+R4}_pooled_s{42,43,44}.json`, `results/v4/block3_run.log`).
Best pooled-val Macro-F1 values: control **0.5768 / 0.5795 / 0.5879**, composite **0.5740 / 0.6222 /
0.6144**. These are val numbers from max-over-epochs selection. They are **not** the S54 readout.
While training ran, this session stayed off torch. Torch work (the CPU smoke) started only after the
log said `Block 3 complete`, and a process check confirmed no `train_v4` was still alive. No test
read, and **no reserved read**: `results/v4/s54/reserved_receipt.json` does not exist.

**Plan frozen before any read.** The plan is `results/v4/s54_plan.json`, sha256
**`990320b6ce22f92a…`**. The digest is taken from the file, with newlines pinned (the S51 lesson).
The ledger row is `v4_s54 / S54_plan`. The plan embeds `recipe_ladder_plan.json → s54_gate`
verbatim and pins that file's sha256. `require_plan()` refuses if either file changes. It fixes
the four open choices:
1. **`_last.pt` is primary** (runbook rule for multi-stage rungs). `_best.pt` is scored as a
   sensitivity row and never sets the outcome. Integrity check: `_last`'s payload epoch must equal
   the run JSON's `epochs_run`, and `_best`'s must equal `best_epoch`.
2. **No TTA and no calibration for V4.** Each arm is scored with the single deterministic view it
   was selected with (`build_eval_transform`). The V1 comparator is the deployed system (24-view
   TTA + deployed Dirichlet map), so Gate A is additionally biased *against* V4 (declared).
3. **Under-40 escalation pAUC, FPR ≤ 0.20, McClish** (`frontier.partial_auc`), computed on
   escalation mass. Only `<40` rows are used, and the bootstrap resamples that band's groups (S51's
   convention). Full-coverage Macro-F1 uses all 4,733 rows over 7 classes.
4. **Seed-mean paired delta with its range, never the best seed.** The estimate is
   `mean_s [metric(composite_s) − metric(control_s)]`. Its interval comes from a lesion-grouped
   bootstrap (`group_id`, 2000 resamples, seed 42) that applies **one resample to all models**.
   With one seed this equals `oos_probe.paired_delta_ci` exactly (asserted). Per-seed intervals are
   that function's output.

"Moves" = lower bound > 0. Upper bound < 0 is reported as **harm**. **Gate B decides** (`_last`,
seed-mean). **Gate A** is emitted only as the deployment delta and is labelled *CONFOUNDED* in every
row. A missing seed refuses the gate; it is never read from fewer than three seeds.

**Built** (`research/v4/`):
- `s54_guard.py` (torch-free) implements the **read-once receipt**
  (`results/v4/s54/reserved_receipt.json`). There are three stages: `v1_topup`, `infer`, `gate`.
  A completed stage refuses to run again without `--rerun-reason`, which is kept permanently. An
  unfinished stage resumes with `--resume` and skips items already hashed. The receipt pins the plan
  hash. Host checks:
  - an OS file lock (released when the process dies, so it can never go stale);
  - refusal while any `train_v4` / `run_block3.ps1` / `run_morning.ps1` process exists;
  - all six runs must be banked (their end-of-run JSON exists);
  - disk ≥ 5 GB;
  - commit headroom via `GlobalMemoryStatusEx`: ≥ 4 GB for torch stages, ≥ 2.5 GB for the gate.
- `s54_infer.py` loads each checkpoint's saved recipe and `image_size` and checks them against
  `compose()` and the plan (control 224 px, composite R1+R4 384 px). It scores through
  `train_v4.V4Dataset` + `evaluate` unchanged and writes
  `results/v4/s54/predictions/{run}_{last,best}.csv`, hashed into the receipt.
- `s54_topup_v1.py` scores the **146 reserved `scc` images** (all BCN) through S13's own path
  (`extract_one` + `_cohort_config` + `_attach_metadata`, scales 0.9/1.0/1.1, batch 4) and S14's
  assembly (`load_member_matrix` + `assemble`). It writes to `results/v4/s54/v1_topup/`, never over
  the hashed S13 files. `--dry-run` (image ids only): frozen V1 covers **4,587 / 4,733**, the
  top-up adds **146**, overlap **0**.
- `s54_gate.py` handles the plan, comparator loading (full coverage asserted, duplicate sources
  refused, V1 `true_index` checked against `class_index_7`), the cohort check (4,733 images / 1,992
  groups / 104 under-40 escalating lesions / 279 images, asserted before any prediction is read),
  both gates, the outcome, and ledger rows under `v4_s54` (pruned by method, the S20 hazard).

**Verification, all run here.**
- `s54_gate --selftest`: **16/16**. It covers: fast Macro-F1 equals sklearn with an absent class;
  the one-seed interval equals `paired_delta_ci`; synthetic outcomes 1/2/3/4 and harm;
  seed-mean = mean of per-seed deltas; five receipt rules; `ham_test` refused.
- Every guard was shown to fire: a second process is refused by the lock; a dummy process carrying
  the `train_v4` marker is refused; an unbanked run is refused; the real gate refuses before
  `v1_topup` has completed and creates no receipt.
- **CPU smoke on V4 val.** The top-up scored val's 59 `scc` images, then re-scored 8 non-`scc` val
  images that S13 had already scored. Against S13, max |Δp| per member is **3.1e-4 … 2.7e-3**
  (ensemble 5.9e-4), with **argmax identical on every row**. The first smoke *failed* its 1e-3
  tolerance on `efficientnet_b3` and `resnet50`. The TTA engine has no AMP, and S13 ran on CUDA,
  where cuDNN convolutions use TF32 by default. So the tolerance is now device-aware: **1e-3 on
  CUDA** (where the real run happens) and **5e-3 on CPU**, with identical argmax required on both.
  This is a measured device effect, not a relaxed check. A pipeline error moves probabilities by
  more than 1e-2. `s54_infer --smoke` scored 48 val images × 12 checkpoints (~9 s at 224 px, ~11 s
  at 384 px each, CPU), with every epoch-integrity check passing. `s54_gate --smoke` ran end to end.
  Val has **zero** under-40 escalating images (S48 routed them all to reserved), so the smoke reads
  pAUC on all ages. **No smoke number is a finding.**

**⚠️ For the S54 reader: `_last` here means "8 epochs past the val peak".** Every pooled run
early-stopped at patience 8 (best/last epochs: control 7/15, 14/22, 6/14; composite 8/16, 11/19,
11/19). Every `_best` epoch is ≥ 6, i.e. inside the fine-tune stage (the head stage is epochs 1–3),
so S45's "`_best` picks a pre-intervention epoch" trap **does not occur in these runs**. The plan
nevertheless follows the runbook, as instructed. On the 48-row smoke the two choices already gave
different outcomes (4 vs 2), which is noise at that size but shows the choice can matter. Read the
`_best` sensitivity row alongside the outcome. If `_best` is to be primary, the plan must be
re-frozen **before** `v1_topup` runs; after that, the receipt refuses a changed plan.

**Timing.** Measured on CPU: the top-up takes ~45 s per architecture for 59 images, and V4 scoring
takes ~0.2 s per image at 384 px. **GPU time is not measured.** Each step prints seconds per item.

### S54 pre-read audit — every S53 metric re-derived; expectation written down before the read (2026-09-16, night)

No test read, no reserved read, no GPU. `results/v4/s54/reserved_receipt.json` still does not exist.
`_last` stays primary, as the plan declares.

**Static audit of all 13 runs and 26 checkpoints: 0 failures.** For each run in
`results/v4/recipe_runs/*.json`, the audit checked the following:
- **Run metadata:** `run_id` matches the filename; the plan sha matches the frozen
  `eff9d79f36f351ff…`; recipe == `compose(rungs)`; effective batch is 32; not smoke; no test read;
  device is CUDA.
- **History:** it is contiguous; `best_val_macro_f1` and `best_epoch` equal the first argmax of the
  history; `final_*` equals the last row; the stop is consistent (either patience 8 past the best
  epoch, or the full schedule); no best epoch falls in the head stage.
- **Checkpoints:** each checkpoint payload's epoch, `monitor_value`, recipe, `image_size`, seed,
  `run_id`, plan sha and history prefix match the JSON. All 26 `state_dict`s are pairwise distinct,
  so no checkpoint was overwritten by a sibling.
- **Pooled grid:** complete (2 arms × seeds 42/43/44), all runs on 15,294 train / 2,270 val images.
- **Peak VRAM:** 2.41 GB at 224 px, 3.49 GB at 384 px (batch 16 × accum 2).

**Defect 1, fixed: every S53 ledger row described two checkpoints.** `train_v4.write_ledger` logged
`macro_f1` from the best epoch, but `accuracy`, `balanced_accuracy`, `escalation_sens` and
`missed_serious` from the final epoch, which is up to 8 epochs later. All metric columns now come
from the best epoch, and the notes say `metrics_epoch=best`. The 13 existing `v4_s53` rows were
corrected in place from the run JSONs, keeping their timestamps and order; the diff touches only
those rows. Example: `R6_ham_only_s42` balanced accuracy 0.7648 → 0.7717, escalation sensitivity
0.7955 → 0.7987.

**Defect 2, recorded rather than rewritten: R4's screen endpoint compares non-comparable epochs.**
`screen.py` computes R4's shift as `final − final`. R0's final epoch is epoch 23, eight epochs past
its peak. R4 ran the full 30 epochs and peaked at epoch 29. The frozen plan does not say which epoch
the endpoint uses. The same R4 − R0 escalation-sensitivity delta, read at different epochs:

| comparison | Δ escalation sensitivity |
|---|---:|
| final vs final (what `screen_verdict.json` reports) | **+0.0617** |
| best vs best | **+0.0032** |
| same epoch 15 / 23 | +0.0617 / +0.1006 |
| mean over matched fine-tune epochs 4–23 | **+0.0719** (positive at 16 of 20 epochs, range −0.065 … +0.175) |

The matched-epoch view does support R4 moving the operating point, so the composite is defensible.
But the single +0.0617 is one reading from a curve that swings ±0.1 from epoch to epoch (R0: 0.66 →
0.90). Referral rate is unmatched as well. Treat the number as directional. Screen verdict, plan and
composite are left untouched; the composite is already trained.

**Pooled-val preview (not the S54 readout; val numbers, selected on val).** Composite − control,
seed-paired:

| seed | Δ best | Δ last | matched-epoch mean ΔF1 | matched-epoch mean ΔEsc |
|---|---:|---:|---:|---:|
| 42 | −0.0029 | −0.0096 | −0.0061 (5/12 epochs > 0) | +0.0747 |
| 43 | +0.0427 | +0.0150 | +0.0079 (12/16) | +0.0710 |
| 44 | +0.0265 | +0.0230 | −0.0124 (3/11) | +0.0499 |

Seed-mean Δ best is **+0.0221**, which is at the MCID. Seed-mean Δ last is **+0.0095**, below it.
The matched-epoch mean is **≈ −0.004**. The composite's best-F1 seed range (0.574–0.622, width
0.048) is about 4× the control's (0.577–0.588, width 0.011). The Macro-F1 gain lives in the
max-over-epochs statistic. The escalation shift is consistent across all three seeds.

**R1 (384 px) holds up. It is the one Macro-F1 lever that is not an artefact of epoch selection**
(HAM val, seed 42, `R1_ham_only_s42.json` vs `R0_ham_only_s42.json`):
- **A plateau, not a spike.** R1's last five epochs read 0.7960 / 0.7971 / 0.7981 / 0.7979 / 0.7974.
  R0's 0.7773 is a single spike; its next-best epoch is 0.7598.
- **Clears the control repeatedly.** 11 of R1's 30 epochs reach or exceed R0's best, and its
  `_last` (0.7974) beats the reference trainer's best (0.7765, `control_reference.json`).
- **Matched epochs 4–23:** R1 − R0 averages **+0.0157** (median +0.0161, positive at 15/20).
- **Caveats:** one seed; HAM val only; R0 early-stopped at epoch 23 while R1 ran all 30. The pooled
  composite dilutes R1 with R4 and a BCN/MSKCC val set, so the pooled preview above neither confirms
  nor refutes it.

**Expectation, stated before the read.** This is not a pre-registration; the plan is frozen and
unchanged.
- Gate B (a), full-coverage Macro-F1 on `_last`: most likely **does not move**. The `_last` val
  delta is below the MCID, and the reserved cohort is lesion-grouped at n≈2k.
- Gate B (b), under-40 pAUC: R4 is a band-constant resampling. S36 check 7 proves it cannot change
  within-band ranking, so any (b) movement has to come from R1's resolution. S51 found no
  representation lever that moves under-40 pAUC.
- Most likely overall: **outcome 4 ("neither")**, with a `_best` sensitivity row that looks better
  on (a) than `_last`. If the read instead returns 1–3, this paragraph is the record that it was not
  expected.

**Host at hand-off:** C: 33.5 GB free; commit 33.9 GB limit with 5.6 GB free (guard needs 4); GPU
idle, 765 MiB used; no `train_v4`/Block processes running. Plan sha256 `990320b6ce22f92a…` and
ladder sha256 `eff9d79f36f351ff…` both verified from their files. `s54_gate --selftest` passes
16/16 (re-run).

**First launch refused, correctly; the commit guard was fixed.** `s54_topup_v1` stopped twice at
preflight: headroom was 3.15 and then 3.18 GB, below the 4 GB floor. A bare check in between read
5.87 GB. No receipt was written. The two readings differ because the guard measures *inside* the
scoring process, after that process's own torch import has already been charged. Measured here:
- **main process:** 5.73 GB bare → 3.13 GB after importing `s54_topup_v1` → 2.88 GB after CUDA init;
- **each spawned dataloader worker:** a further **2.66 GB**.

So the default of 2 workers needed ~8.2 GB against ~5.8 GB actually free. The fixed floor was the
wrong model in both directions: it refused a run that fits with zero workers, and it would have let
2 workers start with only ~4 GB free. `s54_guard.torch_commit_gb(n) = 1.5 + 2.7·n` replaces
`MIN_COMMIT_HEADROOM_GB_TORCH`, and `s54_infer` / `s54_topup_v1` now default to
`--num-workers 0`. The plan does not pin a worker count, and workers cannot change predictions:
the evaluation transform is deterministic and `s54_infer` asserts row order. Verification:
in-process headroom 3.20 GB against 1.5 GB required; `s54_gate --selftest` 16/16;
`s54_infer --smoke --device cpu --limit 8 --seeds 42` → 4 files, no receipt.

### S54 results — the reserved cohort read once: **outcome 4, "neither"** (2026-09-16, night)

**Receipt.** `results/v4/s54/reserved_receipt.json` records `v1_topup` 1, `infer` 1, `gate` 1, with
no rerun reasons. The two GPU stages were run by hand at 20:20–20:42 with `--num-workers 0`. The
gate was then run once. Outputs: `results/v4/s54/s54_gate.json`, `s54_contrasts.csv`,
`s54_marginals.csv`, plus 23 ledger rows under `v4_s54`. `_last` stayed primary, as frozen. The
owner raised the `_last`-vs-`_best` question after both GPU stages had completed, so the receipt had
already locked the choice. Both rows are reported below.

**Cohort, asserted by the gate and re-derived independently:** 4,733 images, 1,992 lesion groups,
279 under-40 escalating images from **104** lesions. V1 covers all rows (4,587 frozen + 146 top-up).

**Gate B, composite (R1+R4) − control (R0), seed-mean paired, lesion-grouped 95% CI:**

| checkpoint | full-coverage Macro-F1 | under-40 pAUC@0.20 | outcome |
|---|---|---|---|
| **`_last` (primary)** | **−0.0121 [−0.0321, +0.0098]**, seeds −0.0335 … +0.0107 | **+0.0089 [−0.0200, +0.0367]**, seeds +0.0039 … +0.0119 | **4 — neither** |
| `_best` (sensitivity) | +0.0316 [+0.0036, +0.0563], seeds +0.0152 … +0.0563 | −0.0106 [−0.0367, +0.0161], seeds −0.0289 … +0.0046 | 2 — does not agree |

**Why the two rows disagree.** The checkpoint rule moves the *control* more than the composite. On
reserved, the control's `_best` is *worse* than its `_last` for seeds 43 and 44 (0.5754 vs 0.6091,
0.5537 vs 0.6111; seed means 0.5754 vs 0.6027). The composite moves the other way (0.6070 vs
0.5906). Within one arm and seed, swapping the checkpoint shifts reserved Macro-F1 by up to
**0.057**, which is larger than either Gate B estimate. **The Macro-F1 conclusion depends on the
checkpoint rule and must be reported that way.** Reading `_best` as a win would select on the
sensitivity row.

**The under-40 conclusion does not depend on the rule.** Neither row moves pAUC; the point estimates
are +0.009 and −0.011. Per-model under-40 pAUC sits in **0.720–0.750** for every V4 model and is
**0.7335** for V1. This agrees with S51's 0.73 control on the same cohort.

**Gate A, composite − deployed V1 (CONFOUNDED, deployment delta only):**
- **Macro-F1: +0.1798 [+0.1297, +0.2254]** on `_last` (+0.1963 on `_best`). V1 scores 0.4108 on
  reserved (BCN 0.4057, MSKCC 0.2557); V4 scores 0.57–0.62.
- **Under-40 pAUC: +0.0038 [−0.0525, +0.0615]**.
- This gain comes from V4 having been trained on these archives, not from the recipe. It is also
  biased *against* V4 (V1 gets 24-view TTA + Dirichlet; V4 gets one view).
- **This is the most informative contrast of the session.** Training on the target archives lifts
  full-coverage Macro-F1 by about 0.18 and leaves under-40 escalation ranking where it was
  (0.73 → 0.73). The under-40 failure is **not** a domain-coverage or data-coverage problem.
  Together with S51 (no public backbone lifts it) and this gate (no recipe lifts it), the
  representation route is closed three ways.

**Independent verification.** A separate script, not the gate's code, recomputed Macro-F1
(`sklearn.f1_score`, labels 0–6) and under-40 pAUC (`roc_auc_score(max_fpr=0.2)`, McClish) for all
12 V4 files and for V1. All 12 per-seed deltas and both seed means match the gate to 4 decimals.
V1: 0.4108 / 0.7335. Every file was also checked for row order and labels identical to the first,
probabilities summing to 1, and `pred_index` equal to the argmax. This re-reads the frozen
prediction files only; no model was re-scored.

**Pre-read expectation vs result.** The prediction was outcome 4 with a better-looking `_best` row.
That is what happened, so the result is not a surprise read post hoc. R1's val plateau (see the
pre-read audit) did not become a composite gain on reserved. The composite cannot separate R1 from
R4, so this does not refute R1 on its own.

**Consequence (runbook outcome 4):** the representation is not reachable by recipe, and §4 (S55–S59,
the mechanism) is the whole contribution. That is still a deployable result. **⛔ Checkpoint: stop
and review before S55.**

## 11. Known findings that constrain later work

- **No better representation is available off the shelf, so V4's gains must come from the
  recipe and the decision layer — not from a new backbone** (S51,
  `results/v4/backbone_probe_deltas.csv`). A derm foundation model (PanDerm ViT-B/16) and a
  general-purpose one (DINOv2 ViT-B/14), both frozen and probed with S41's **unchanged**
  instrument on 104 under-40 escalating lesions, come in at **0.6956** and **0.6593** under-40
  pAUC against the in-repo trunk's **0.7319**. The pre-registered falsifier (≥ +0.05, CI
  excluding zero) fired on a **−0.0363 [−0.0828, +0.0127]**. H2's "the features are the limit"
  now has a second reading: they are the limit *and* nothing publicly available lifts it, so
  §9 rule 6 ("build no ninth architecture before S51 says the representation is the
  bottleneck") is now settled in the negative and binding. PanDerm's CC-BY-NC licence
  consequently never becomes a product constraint.
- **The age shortcut is peri-lesional in direction but not in magnitude, and R3 is demoted on
  that distinction** (S50, `results/v4/shortcut_locus.json`). Exterior-vs-interior paired Δ on
  `age_band` is **+0.0297 [+0.0174, +0.0424]** and the selectivity margin (Δage − Δescalation) is
  **+0.0332 [+0.0147, +0.0522]** — both intervals exclude zero, both fall short of the declared
  MCID of 0.05. Three things constrain anyone who reopens this: (1) a `CO_LOCATED` verdict is
  **instrument-limited**, since receptive-field bleed shrinks the contrast toward zero, so it is
  not evidence that age is genuinely co-located — though `control_bleed` shows exterior age AUC
  is **flat in lesion area** (+0.0047 [−0.0207, +0.0301]), which bleed alone cannot produce;
  (2) **lesion area alone predicts age band at 0.5483 [0.5368, 0.5599]**, so part of the signal
  is geometry that no mask removes; (3) the `<40` band's exterior advantage is **+0.052**,
  the only band reaching the MCID — post-hoc, no interval, in no family, and it **must be
  pre-registered before extraction** if S51 wants to test it. Separately, `mask_augment.py`
  measures that a lesion-centred crop is a **no-op on 18.9%** of usable lesions, which bounds
  R3 independently of the probe.
- **A 38-row preprocessing choice flips the R3 gate, and macro-OvR is why** (S50). S42's
  `age_band` admitted `age_band='unknown'` as a fourth class; at 0.55% of rows it still carries
  25% of a macro-OvR average, and it alone supplies **60%** of the Δ that would have promoted R3
  (`+0.0507`, verdict `PERI_LESIONAL`) instead of demoting it. Any future macro-averaged probe in
  this repo must state which classes it admits and why — `BANDS` in `research/v3/probes.py`
  already excludes `unknown`, and the S42 run simply did not filter on it.
...
**Correction made alongside this document**: The session-4 conformal line previously read
...
`research/conformal/results/session4_conformal_report.md` and now recorded here.
...
## 16. Review-1 demonstration report (`review1_infographic.html`) — rebuilt 2026-09-14

**No test read.** No `results/` artifact was written, read for the first time, or recomputed;
this session only re-presents numbers that already existed on disk. The test-pass receipt stays
at `n_executions: 2`.

### Why it was rebuilt
The previous version of the file had drifted from what a reviewer-facing demo needs:

1. **Team & governance had been demoted.** It existed only as a card at the bottom of section 01,
   below six findings. In the original tracked version (`7c12689`) it was a prominent standalone
   section. It is now **section 02**, full width, with a cover band, ownership table mapped onto
   the architecture, the supervision/examination panel, and a three-step description of the
   working process.
2. **The provenance footer rendered under every tab.** The prev/next bar and the long provenance
   paragraph sat after `</section>` of the last tab, outside the tab system, so they appeared
   below whichever section was active. Now: one compact prev/next bar inside the tab area, and the
   provenance paragraph in a single document `<footer>` outside `<main>`, rendered once.
3. **"Bugs found, and what each one would have cost" is gone.** Its content was not discarded —
   it is reframed as **"Threats to validity, and the control that caught each one"** (§21), a
   3-column table of *threat → mechanism that would have produced a wrong number → control and
   what it changed*. Nine rows, covering leakage, in-sample feature extraction, test-set
   selection, the missing iso-referral comparator, parameter drift between modules, re-implemented
   scorers, the silent DeLong failure, the retired endpoint, and the over-claiming captions.
4. **Zero figures were embedded.** `grep -c '<img'` returned 0 against ten rendered PNGs sitting
   in `paper/figures/`.

### What the file is now
- **21 sections** (was 18), grouped Foundation / evidence chain / falsification / synthesis.
- **New section 03 — System architecture.** An inline SVG of the full inference stack marking
  which parameters are *learned* (the six CNNs) versus *fitted on OOF/val and frozen* (everything
  downstream); twelve stage cards with their parameter counts and source modules; a backbone
  specification table; the inference-cost derivation (6 × 24 = **144** forward passes per image,
  720 for the rejected A8 fold bag); and a module map. Parameter counts are labelled as published
  `timm` reference values, **not** as `results/` artifacts, to keep Hard Rule 4 intact.
- **New section 18 — Explainability**, carrying `figure7_gradcam.png` plus the attribution numbers
  from `results/session9/attribution_test.csv` and the explicit statement that errors score
  *higher* (0.570) than correct predictions (0.513), i.e. the predicted-class artefact.
- **All nine available figures embedded** by relative path with an `onerror` fallback that names
  the missing file: `figure1_architecture`, `figure2_reliability`, `figure4_selective`,
  `figure5_conformal_coverage`, `figure6_abstention_tradeoff`, `figure7_gradcam`,
  `external_figure_dose_response`, `external_figure_decision_curve`,
  `external_figure_case_atlas`. **The HTML must stay at the repository root** for these to resolve.
- **19 Chart.js charts (was 5), across 10 chart types** — grouped bar, horizontal bar, stacked bar,
  diverging bar, floating-bar interval, line, dual-axis line, scatter, path-scatter, bubble,
  doughnut, radar. Each series is transcribed from the artifact named beside its canvas.
- **11 hand-authored inline SVG explainers** — the one-picture project summary, the full
  architecture, the leakage trap, the under/over-confidence gap, the 16:0 one-directional
  calibration flips, the per-band abstention dot rows, the 290-lesion dot grid, the conformal
  false-reassurance set, the λ-rule number line, the adversarial age-loss bar, and the eight-node
  decision chain. All theme-aware via CSS custom properties.
- **A consistent explanation layer in every section**: an "In plain terms" block (plain-language
  reading of the numbers above it) and, where a causal story is being made, a three-card
  *setup → what happens → why it matters* mechanism strip.

### Defects found and fixed while verifying the rebuild
- **Cross-section SVG marker reference.** The calibration-antinomy SVG referenced `url(#ar1)`,
  a marker defined inside section 01's SVG. Sections are `display:none` when inactive, so the
  arrowhead would have failed to paint. Each SVG now defines its own marker.
- **Horizontal page overflow.** Measured `document.scrollWidth` 1103 against `innerWidth` 1024.
  Cause: grid items default to `min-width:auto`, so a wide `.tbl` forced its grid column past the
  viewport instead of letting `.tw` scroll internally. Fixed with
  `.grid-2>*,.grid-3>*,.grid-4>*{min-width:0;}`. Re-measured: **0 overflow in all 21 sections at
  both 1024 px and 375 px**.
- **Overlapping labels in the 290-lesion dot grid.** Band captions collided with the first row of
  squares. Geometry reworked (left label column widened to 210 px, per-band vertical offsets
  respaced, viewBox 300 → 318).

### Verification performed
Served locally and driven in a browser: 21/21 sections present and each reachable from the rail
and the pill nav (no orphan ids, no dangling nav targets); **19/19 charts instantiate with no
exceptions**; **9/9 images load** (`naturalWidth > 0`); no console errors; light and dark themes
both checked; every SVG `url(#…)` resolves inside its own `<svg>`; tag balance verified for
`section`/`figure`/`table`/`svg`/`main`/`footer`/`div`. The pre-rebuild file is preserved at
`scratchpad/backup_original.html` for this session only.

### Open
The page count and abstract-length items from S17/S19 are untouched by this session — they belong
to the manuscript, not to this document.

### S52 morning run — control gate resolved, Block 2 launched (2026-09-16, unattended, 13:02–14:35)
...
not resolved**: Block 2 (below) is running unattended right now on infrastructure the owner had
removed the automation for.

**Bug found and fixed, `scripts/run_morning.ps1`.** `$ErrorActionPreference = "Stop"` at file scope,
combined with `python ... 2>&1 | ForEach-Object { Log $_ }`, meant PowerShell 5.1 wrapped the
first stderr line any python call wrote (a tqdm progress line, not an error) in a terminating
`NativeCommandError` and killed the script — before its own `$LASTEXITCODE` check ever ran. This is
why the prior entry found no `morning_run.log` at all: the script never survived past its first
native call. Every failure path in the script already branches on `$LASTEXITCODE` explicitly, so the
file-scope preference was redundant with that and actively wrong; changed to `"Continue"`.
Root-caused by reproducing it directly (`Start-Job` capture showed the script reaching
`--- stage head ---` then dying on the first progress line) before touching the file.

⚠️ **Self-inflicted collision, real, while finding the bug — and it needed operator intervention to
fix.** Debugging the `$ErrorActionPreference` bug involved four separate launches inside ~10 minutes
(`Start-Job`, two `Start-Process` attempts, one `Bash`-tracked background run); several of those,
believed dead at the time, turned out to be merely delayed and fired later on their own — one was
correctly refused by a busy-check added mid-session (`morning_run.log` 14:37:43, "A training process
is already running"), but a second (15:05:58) slipped past it and reached the point of starting a
*second* copy of the composite `R1+R4` training on top of the one already 3 epochs in. **The
`Bash`-tracked run (this session's own `run_in_background`) was training inside this session's
sandboxed container — fragile for a multi-hour job, since it does not outlive the session.** The
operator caught this and intervened directly, logging it in-band
(`morning_run.log:32167`): *"STOPPED BY OPERATOR: composite R1+R4_pooled_s42 (batch 32, 2 workers,
inside the app container) is being restarted outside the container with batch 16 x accum 2,
1 worker. ~10 min of composite progress discarded; nothing banked is affected."* Confirmed nothing
banked was overwritten: R1, the control reference, and the R0-pooled-control JSON were all already
written to disk before the collision and are single, self-consistent runs. The composite `R1+R4`
arm now training is the operator-restarted one, outside this session's container, and is what should
be trusted. Independently of this, the script gained a named mutex
(`Global\capstone_v4_run_morning`), a process-level busy-check before Block 2, and a retry-on-
`FileShare.ReadWrite` log append. Take those as current state, not something to revert — they
directly address the failure mode above, and the busy-check is what caught the 14:37:43 attempt
(imperfectly — see above).

**R1 re-run** (`results/v4/recipe_runs/R1_ham_only_s42.json`, batch 16 × grad-accum 2 = effective 32,
1 worker — the reduced-memory settings from the overnight commitment-limit failure): best val
Macro-F1 **0.7998** at epoch 25/30 (final-epoch 0.7974), 3,005.8 s (~50.1 min), VRAM peak 3.49 GB of
8.55. Delta vs R0 **+0.0225** — clears the 0.02 MCID.

**Control-gate diagnostic resolved** (`results/v4/control_reference.json`,
`results/v4/screen_verdict.json`). `ml/training/train.py` (the original trainer) scored **0.7765**
on the identical split, epoch 25/30 — **0.0008** from `train_v4.py`'s R0 (0.7773), inside the 0.02
MCID. Both trainers land above the published band (0.735, 0.765) by essentially the same amount, so
`control_gate` = **`FAILED_BUT_DEVIATION_ACCEPTED`**: the band is stale (a reference-value problem),
not a `train_v4.py` divergence. Confirms the prior entry's own diagnosis (R0's 0.7773 as a
single-epoch spike against a jumpy fine-tune curve), now with a same-split, same-trainer number
instead of a cross-run comparison.

**Block 1 screen, final** (`results/v4/screen_verdict.json`, all 7 arms now on disk). Deltas vs R0
(0.7773): **R1 +0.0225 CLEARS**, R6 −0.0121, R5 −0.0290, R2 −0.1022. R4 clears its own endpoint
(escalation-sensitivity delta +0.0617 ≥ 0.05 MCID; Macro-F1 delta −0.0091, predicted null per S36
check 7). R7 misses its own endpoint (age-flip span 0.0475 < 0.05) and is not non-inferior either.
**Composite promoted to Block 2: R1 + R4.**

**Block 2 launched**, `--corpus pooled --batch-size 32 --num-workers 2`: pooled control (`R0`) first,
then the composite (`R1 R4`), ~1.4 h each at full 30 epochs → projected finish **18:24**, **295 min
past the 13:30 deadline**. The deadline was already unreachable before this leg started (R1 alone
needed 50 min from a 13:02 launch); the script logs the overrun and proceeds anyway, citing every
Block 1 arm's early stopping so far (R0 23/30, R2 15/30) as reason the real finish is likely earlier.
Disk free **26.44 GB** — no repeat of the overnight commitment-limit failure. Block 3 (seeds 43, 44)
was **not** started, per instructions.

### Open
Resolve the scheduled-task mismatch above before anything else touches `v4-morning-*`: either the
owner re-authorizes unattended runs of this pipeline, or the trigger source that fired this session
needs to be found and closed off. Block 2 is running unattended as this entry is written; check
`results/v4/morning_run.log` for whether it finished, and `results/v4/recipe_runs/` for the R0-pooled
and composite (R1+R4) results, before trusting any pooled-corpus number from today.

### S55 — group-conditional (per-age-band) Dirichlet calibration (2026-09-16)

Per the V4 runbook (§4, Phase M): S54 closed the representation route (outcome 4, "neither" —
neither S51's foundation-model arm nor S52-54's recipe ladder moved under-40 ranking), so §4's
group-conditional decision layer is now the whole contribution. S55 is its first piece.

**The problem, restated from S7** (`research/stats/results_oof/band_calibration.csv`): after the
project's one **global** Dirichlet map, the residual signed calibration gap *flips sign* across
age bands — `<40` stays under-confident (−0.027) while `60+` is pushed into over-confidence
(+0.041) — so an aggregate ECE of 0.017–0.025 hides two opposite-signed errors. One affine map
cannot close two gaps pointing in different directions.

**Built**: `research/multical/groupwise.py` (per-band Dirichlet fitting + K-fold cross-fitting
within each band, falling back to the global map for an under-powered band) and
`research/run_session55_multical.py` (the runner). Fit on **OOF only** (6,981 rows; `<40` gets
1,319 of them, `40-59` 3,112, `60+` 2,512 — enough to fit a ~50-parameter map per band, which
validation's 1,532 rows would not be). `unknown` (38 rows) falls back to the global map —
`MIN_FIT_SIZE=300`, chosen because a Dirichlet map at C=7 has on the order of 50 free
coefficients and 38 rows would be overfitting, not multicalibration. No test read; this session
has no published val-fitted twin, so `testguard.block_test_reads` is armed unconditionally.
Leak discipline matches `research.agerule.lambda_rule.crossfit_calibration`: every ECE and
signed-gap number reported is scored by a map that never saw that row, whether it is the
existing global map or the new per-band ones.

**Result — the sign-flip closes, but a different disparity survives** (`results/multical/results_oof/session55_report.md`,
`band_calibration_multical.csv`; all numbers OOF, cross-fitted):

| band | uncalibrated signed_gap | global signed_gap | per-band signed_gap | global ECE | per-band ECE |
|---|---:|---:|---:|---:|---:|
| `<40` | −0.229 | −0.027 | **+0.007** | 0.0276 | **0.0095** |
| `40-59` | −0.190 | +0.005 | +0.008 | 0.0260 | 0.0219 |
| `60+` | −0.119 | +0.041 | **+0.015** | 0.0410 | **0.0301** |
| ALL | −0.172 | +0.012 | +0.010 | 0.0246 | 0.0190 |

The max−min spread of the *signed* gap across bands drops **0.0679 → 0.0084** — the sign
disagreement between `<40` and `60+` is essentially gone, and every band's own ECE improves
individually. **But the ECE *spread* across bands does not shrink** (0.0150 → 0.0206, wider, not
narrower): `<40` had the most sign-flip room to close and improves the most (−0.018), leaving
`60+` (−0.011, to 0.030) the worst-calibrated band by a wider margin than before. *(The final
audit corrected an earlier "barely moved": `40-59` moved least, −0.004.)* Per-band calibration
fixes the *direction* disagreement, not the *magnitude* one; `60+` remains this project's weakest
calibrated band, now more clearly than under the global map. Reported as measured, not smoothed
over — this is exactly the discipline S9/S19 established for a mechanism that only partly works.

**Frozen artifact**: `research/multical/results_oof/fit_state.json` (per-band deployed Dirichlet
states, fit on each band's whole OOF allocation with no cross-fitting, for a later session to
apply to a split this fit never saw) plus `gap_summary.json`. Per the runbook's instruction that
`research/external/frozen_params.py` remains the sole loader for any frozen transfer parameter,
that module gained `load_group_dirichlet()` and `calibrate_by_band()` rather than a second, local
loader — verified end-to-end (loads, applies to the OOF panel, every row sums to 1) and the
existing `--selftest` (6 checks, global map/lambda/val-Macro-F1 reproduction) still passes
unchanged, confirming nothing about the deployed global map or the frozen age-rule lambdas moved.

**One bug caught before it became a ledger duplicate**: the first two runs of the new runner
appended the same 5 rows twice (the exact hazard S20 flagged twice already — a runner with no
prune step duplicates on every re-run). Fixed by copying
`research.external.eval_age_rule_transfer`'s `prune_prior_rows` pattern into the new runner,
namespaced on `session == "session55_multical"`; verified idempotent (a third run reported
"replaced 5 ledger row(s)" and the file stayed at 5 rows for this session, not 15).

Logged to `research/experiments.csv` under `session55_multical` (5 rows: one per band + ALL).
No manuscript claim depends on this yet — S55 is preparatory for S56 (group-conditional selective
abstention, the runbook's core method) and beyond; whether per-band calibration is adopted as the
project's deployed calibrator, rather than left as a measured alternative, is a decision for
whichever later session first needs calibrated probabilities split by band.
...
**Built**: `research/v4/s56_abstention.py`. Modes: `--selftest` (9 checks), `--select`,
`--freeze-plan`, `--reserved` (with `--smoke` and `--resume`). Outputs go to `results/v4/s56/`.
- **Mechanism.** For each global referral budget R, one score threshold is fit per age band on
  OOF (cross-fitted Dirichlet). Each threshold is the smallest one that meets the
  conformal-risk-control bound `(FN_b+1)/(P_b+1) <= 1-S`. The floor S* is the largest value on a
  0.005 grid that the budget can buy in all three bands at once. Leftover budget goes to one
  global threshold, and `unknown` gets the global threshold only.
- **Comparator.** The status quo: one threshold at the OOF `1-R` quantile, same score.
- **System sensitivity.** A referral counts as a catch:
  (argmax-escalated or referred) / escalating. FRR is 1 − system sensitivity.
- **NNB.** Computed on the flagged set (escalated or referred), so it is an **upper bound** on
  biopsy burden.
- **Plan.** `results/v4/s56_plan.json`, sha256 **`4e142ad2c423a0ec…`**, frozen before the
  reserved read. Receipt: `results/v4/s56/reserved_receipt.json`, 1 execution, completed, no
  rerun reason. **Do not read reserved for S56 again** without `--rerun-reason`.

**Score selection on val** (`results/v4/s56/selection_val.json`). The rule was declared before
val was scored. Pick the best **all-ages** val system sensitivity, averaged over R, among scores
whose retained Macro-F1 is within 0.02 of the best. All-ages is used because val has 22 under-40
cases, and S48 bars decisions on ≤25 positives.
- Results (mean system sensitivity / mean retained Macro-F1): msp 0.8617 / 0.8833, margin
  0.8597 / 0.8797, entropy 0.8578 / 0.8835, esc_risk 0.9201 / 0.8224.
- **`msp` is selected.** The first three are a practical tie.
- **`esc_risk` fails the F1 guard.** Escalation mass on argmax-benign cases gives the highest
  sensitivity but loses 0.061 of F1. It is declared in the plan as a **secondary, descriptive**
  arm.
- ⚠️ **Bug caught before the plan froze.** The first `--select` run inverted `msp`/`margin` a
  second time (`research.selective.scores` already returns "higher = less trustworthy"), so it
  referred the most confident cases first and selected `entropy` for that reason. Fixed. Selftest
  check 9 now asserts the direction of every score. The rule itself was not changed.

**OOF floors S\*** (nominal, CRC-corrected) at R = 10/15/20/25/30%: **0.755 / 0.810 / 0.855 /
0.880 / 0.905**.

**Primary endpoint (reserved; `results/v4/s56/s56_report.json`, `contrasts_reserved.csv`).**
Under-40 system escalation sensitivity, band arm − global arm, at R = 0.20, lesion-grouped paired
bootstrap (2,000 draws): **+0.1541 [+0.1044, +0.2156] → SUPPORTED** (MCID 0.10). The result holds
at every budget, and every interval excludes zero: +0.219 / +0.194 / +0.154 / +0.136 / +0.125.
On 104 under-40 escalating lesions (279 images).

**Headline table at R = 0.20 (reserved, `frontier_reserved.csv`).** The cost is in the table, not
a footnote:

| band | global: referral | global: system sens | band: referral | band: system sens | Δ sens [95% CI] |
|---|---:|---:|---:|---:|---|
| `<40` | 0.232 | 0.609 | **0.454** | **0.763** | **+0.154 [+0.104, +0.216]** |
| `40-59` | 0.331 | 0.687 | 0.357 | 0.704 | +0.017 [+0.006, +0.030] |
| `60+` | 0.539 | 0.775 | 0.385 | 0.674 | **−0.100 [−0.118, −0.082]** |
| ALL | 0.403 | 0.733 | 0.386 | 0.693 | **−0.040 [−0.055, −0.025]** |

- **The under-40 referral rate doubles** (+0.222 [+0.186, +0.255]). This is the honest price, as
  the runbook requires.
- **The mechanism redistributes budget; it does not create sensitivity.** Total referral is
  slightly *lower* (−0.017 [−0.031, −0.004]). The under-40 gain is paid for by `60+`, and
  all-ages sensitivity falls by 0.040. Per-band sensitivity spread narrows from 0.165 to 0.089,
  and under-40 goes from worst band to best.
- Retained Macro-F1: 0.4865 → 0.4646 (−0.022 [−0.046, +0.001]).
- NNB at π = 0.03: all ages 11.5 → 13.6; under-40 8.7 → 15.7. Full π ∈ {0.01, 0.03, 0.05} per
  band is in the CSV.
- ⚠️ **Neither the budget nor the floor transfers to reserved.** A nominal 20% budget realises
  a **39–40%** referral rate in both arms, because the uncertainty distribution shifts off HAM.
  The nominal floor 0.855 is **met in no band**: `<40` 0.763 [0.655, 0.853], `40-59` 0.704, `60+`
  0.674. Across all 15 budget×band cells, **12 are violated** (interval upper bound below S\*),
  3 are below the floor with the interval still reaching it, and **0 are met**
  (`s56_report.json:floor_transfer`; the first version said 10 / 5).
- The CRC bound is an exchangeability guarantee and reserved is not exchangeable with HAM OOF.
  **Report the floor as nominal and the budget as a HAM-calibrated setting, never as a
  certified property on a shifted cohort.**
- Context: V1 at full coverage catches only **0.392** of reserved escalating cases (under-40
  0.341). Every system-sensitivity number here sits on a weak transfer base (S14).

**Secondary `esc_risk` arm (descriptive).** At R = 0.20, under-40 +0.111 [+0.065, +0.167];
under-40 referral +0.128; all-ages referral −0.031; retained F1 −0.024. The under-40 gain is
+0.11 at every budget, with less referral cost than `msp` but a larger F1 cost. It decides
nothing.

**Val (descriptive, 22 under-40 cases).** At R = 0.20: under-40 0.591 → 0.909, referral
0.154 → 0.345. Reserved shows the same direction at about half the size.

**Reading for S57+.**
- Group-conditional abstention buys an under-40 floor **by reallocating referrals across bands**.
  The gain is real and clears the MCID on reserved.
- The 60+ loss and the doubled young-patient referral rate are part of the result.
- Off-distribution, budgets and floors need **target-side recalibration** (a small labelled
  target sample, or shift-aware CRC); OOF thresholds alone are not enough. This is the natural
  input to S59's cascade.
- S57a fits λ(age) on top of this coverage regime, as the runbook's dependency states.

Ledger: 22 rows under `v4_s56` (plan, 10 val, 10 reserved, primary). Verified against a
pre-session copy of `research/experiments.csv`: only appends, no duplicates.

**Open.** S55's code lives in `research/multical/` and `research/run_session55_multical.py`,
outside the `research/v4/` tree that runbook §10 requires. It is left in place because
`research/external/frozen_params.py` reads its frozen `fit_state.json`; flagged for the owner.
...
**Built.**
- `research/v4/lambda_age.py` (`--selftest` 14 checks, `--run`, `--audit-reserved [--smoke]`).
- `research/v4/age_estimator.py` (`--selftest` 6 checks, `--run`).

**Outputs.**
- `results/v4/age_bin_diagnostic.csv`
- `results/v4/lambda_baseline_s5.json`
- `results/v4/lambda_candidates.json`
- `results/v4/age_estimator_report.json` (+ `_state.json`)
- `results/v4/s57a/reserved_audit.json` (receipt `reserved_audit_receipt.json`: 1 execution,
  completed, no rerun reason)
- `paper/figures/lambda_age_diagnostic.png`

**Design points.**
- **Exact fast evaluation.** Adding λ never changes *which* escalating or non-escalating class
  wins, so each row reduces to one comparison: `p_maxE + λ` against `p_maxN`, with numpy's
  argmax tie rule.
  - The self-test proves this equals `lambda_rule.apply_lambda` row for row across all 121
    grid values.
  - A first draft compared `λ > p_maxN − p_maxE`, which differed on 3 rows through floating-point
    rounding; fixed before any fit.
- **Loaded objective.** The loss is expected cost under `build_cost_matrix`, with escalation
  specificity ≥ 0.85 **in each of <40, 40-59 and 60+**. That is the unit S5 applied the floor at.
- **Arms.**
  - Every arm is frozen as an 18-value table at the recorded HAM ages 0…85, which HAM records only
    in 5-year steps.
  - Missing age takes the arm's pooled λ, and never an estimated age.
- **Cross-validation.** Hyperparameters are chosen on held-out cost with **nested** cross-fitting:
  - For outer fold k, λ is fit on the other folds, using probabilities from an inner Dirichlet
    cross-fit over those folds only.
  - Fold k is scored with a map fit on the other folds.
  - Disjointness is asserted, and CV bandwidth, κ and τ are selected by a 1-SE rule toward
    smoother.
- **Dependency graph (audit item 3).** Written into `lambda_candidates.json`, for S57b to cross-fit
  λ itself.
- **Declared deviations.**
  - **D1:** A6 is optimised directly on the step-valued objective with a P-spline ridge penalty.
    `Ridge` is used only for the warm start, because there is no squared-error response to
    regress on.
  - **D2:** fits run at full coverage, not on S56's referral regime, because that budget does not
    transfer.
  - **D3:** A4 and A5 are linear and quadratic *in the softplus link*, so λ ≥ 0 holds.
  - **D4:** every fitted arm is **floor-repaired per band** (see bug 1).
  - **D5:** AP (one pooled λ) is a CV reference outside the family.

**Phase 2 — baseline reproduced, loaded not typed.** `frozen_params.apply_age_rule` on S5's
cross-fitted OOF reproduces S5's stored per-band fit sensitivities exactly: <40 0.6250,
40-59 0.8967, 60+ 0.8331.

| Split | Arm | Sens (all) | Missed | Macro-F1 | Bal. acc | Spec | Referral | NNB π=0.03 | <40 sens | <40 referral |
|---|---|---|---|---|---|---|---|---|---|---|
| OOF (in-sample for λ) | A0 | 0.690 | 421 | 0.7776 | 0.752 | 0.955 | 0.170 | 3.11 | 0.547 | 0.052 |
| OOF (in-sample for λ) | **A1** | **0.842** | **214** | 0.7670 | 0.765 | 0.883 | 0.258 | 5.48 | **0.625 [0.495, 0.743]** | 0.076 |
| Val | A0 | 0.714 | — | 0.7638 | — | — | — | — | 0.545 (22 cases) | — |
| Val | A1 | 0.847 | 47 | **0.7713** | — | — | — | 5.86 | 0.591 | — |

- Val Macro-F1 under A1 is **0.7713**, which equals the `frozen_params` self-test constant.
- ECE is 0.0246 (OOF) and 0.0253 (val). λ does not change probabilities, so ECE is the same in
  every arm.
- **This is S57b's comparator.**

**Phase 1 — 5-year diagnostic (OOF).**
- **Escalating cases under 40 are sparse.** There are 64 in total: 0, 1, 0, 0, 1, 9, 21 and 32
  across the bins 0–4 … 35–39.
- **Only 35–39 clears S5's 30-positive gate.**
- **Audit item 1.** 7 of the 8 under-40 bins fall back to the pooled λ **0.65**, which is
  2.5× the frozen <40 value of 0.26. The gate's fallback therefore gives the sparsest bins the
  *strongest* correction.
- **Ranking is weaker in the 30s.** Escalation-mass AUC is 0.872 [0.733, 0.993] at 30–34 and
  0.906 at 35–39, against 0.935–0.957 at 40–69.
- **Argmax sensitivity is flat at ~0.60–0.63 from 35 to 64**, then rises to 0.76–0.79 at 65–74.
- **Where the data exist, the bin-optimal λ has clear structure inside the older bands:**
  - It rises from 0.54 (40–44) to **0.83 [0.63, 0.86]** (50–54).
  - It falls to **0.04 [0.00, 0.23]** at 80+, whose interval does not overlap 50–54's.
  - 35–39 is 0.19 [0.00, 0.76], uninformative.
  - **The resolution exists at 40+ and not under 40, where the problem is.**

**Phase 3 — candidates.** OOF numbers are in-sample for λ. CV cost is leak-free.

| Arm | CV cost ± SE | OOF <40 sens | OOF <40 referral | Held-out spec 60+ | Note |
|---|---|---|---|---|---|
| AP (ref) | 0.341 ± 0.028 | — | — | **0.754** | cheapest only by breaking the 60+ floor |
| A0 | 0.616 ± 0.025 | 0.547 | 0.052 | 0.912 | |
| **A1** | **0.398 ± 0.031** | 0.625 | 0.076 | 0.849 | |
| A2 | 0.431 ± 0.012 | 0.672 | 0.121 | 0.855 | gate fallback puts 0.65 on 7 under-40 bins |
| A3 | 0.406 ± 0.030 | 0.703 | **0.152** | 0.850 | h = 100, **still the grid edge** |
| A4 | 0.438 ± 0.022 | 0.641 | 0.099 | 0.851 | |
| A5 | 0.417 ± 0.022 | 0.625 | 0.080 | 0.850 | |
| A6 | 0.410 ± 0.028 | 0.625 | 0.084 | 0.847 | κ = 1e-3 (1-SE; minimum at 1e-5) |
| A7 | 0.416 ± 0.027 | 0.625 | 0.084 | 0.850 | τ = ∞ (1-SE; minimum at 500) |

**Findings.**
- **No finer-resolution candidate beats A1 on the fitting objective under leak-free CV.** A1 has
  the lowest point estimate in the family. A3–A7 lie within one SE of it; A2 (+0.033) and A4
  (+0.040) are more than one SE worse. *(The first version said every arm was within one SE.)*
- **A7, the runbook's primary candidate, collapses onto its A6 prior (τ = ∞).** The 5-year local
  estimates add nothing out of sample (τ sensitivity is in `lambda_candidates.json`).
- **A6's curve peaks at ~0.71 at ages 45–50 and falls to ~0.3 at 70+.** It gives ages 0–25 a λ
  near zero, so under-40 sensitivity is unchanged from A1.
- **A3 raises under-40 sensitivity only by buying referrals.** At its CV bandwidth it is a
  two-step rule: ~0.66 below 60 and 0.33 at 60+. Under-40 sensitivity goes 0.625 → 0.703, while
  under-40 referral **doubles** (0.076 → 0.152) and <40 specificity drops 0.952 → 0.876. Gate 4
  would name this.
- **Val (descriptive, 22 cases).**
  - A2 and A3 reach under-40 sensitivity 0.682; the others stay at 0.591.
  - A1 already sits **below the 60+ floor on val (0.846)**. A4 ties it (0.846); the other
    candidates are lower (A2 0.819, A3–A7 0.822–0.843).
  - In CV, A1 and A6 fall just under the floor on held-out 60+ data (0.849, 0.847). Floors met
    exactly in-sample hold only about half the time out of sample.
- **Reading for S57b.** On development data this points at Failure A: the resolution is not in the
  data under 40. S57b's bootstrap and gates decide; S57a declares no gate.

**Audit item 2 — reserved, specificity only.** The deployed 40-59 λ **violates the 0.85 floor on
reserved: 0.748 [0.718, 0.775]** (Clopper–Pearson; image-level, lesion clustering ignored).
- 60+ is also below the floor at 0.827; <40 is 0.960 and all ages 0.833.
- Under argmax, 40-59 specificity is 0.978, so λ = 0.74 costs 23 points of specificity off HAM.
  On HAM val it held at 0.856.
- **This is a finding about the deployed rule, not a reason to relax the floor.** It repeats S56's
  pattern: HAM-fitted operating points over-refer on reserved.

**Age estimator** (frozen ConvNeXt OOF features, 6,943 recorded ages, cross-fitted on S3 folds,
penalty chosen by inner lesion-grouped CV).

| Model | MAE (years) | R² | Spearman ρ | Deviance explained |
|---|---|---|---|---|
| Mean-only | 13.79 | — | — | — |
| Ridge (least squares) | 11.82 | 0.219 | — | — |
| **Tweedie GLM (p = 1.5, log link)** | **11.63 [11.36, 11.90]** | **0.249** | 0.516 | 0.195 |

- **The Tweedie GLM beats least squares:** MAE −0.19 years [−0.26, −0.11].
- **It regresses hard to the mean.** Bias is **+19.5 years under 40**, +2.6 at 40-59 and −13.0 at
  60+. Only **3.8%** of under-40 patients are predicted into their own band, and band balanced
  accuracy is 0.434.
- **It is least usable exactly where the project needs age most.** Used as a fallback, it would
  send most young patients to the 40-59 λ.
- **QC flag.** The threshold |residual| > 29.0 years (95th percentile) flags 14.9% of under-40
  cases, against 0.3% at 40-59.
- **The fallback rule holds.** `resolve_age` never overwrites a recorded age (self-tested); the 38
  missing-age rows are the only ones estimated.

**Bugs caught in-session.**
1. **The first fit let A3 beat A1 only by breaking the floor** (60+ specificity 0.77).
   - Cause: A3's kernel fit enforced the floor on *kernel-weighted* specificity, and A7's
     shrinkage had no constraint at all.
   - Fix: `repair_floor` lowers a violating band's λ by the smallest uniform delta, applied to
     every arm and inside CV. Its effect is reported per arm; for A3 it is −0.31 in 60+.
   - The AP reference arm makes the cause visible.
2. **Both estimator penalties first hit the top of their grids** (Tweedie α = 1 in 5/5 folds,
   Ridge 1e4 in 3/5).
   - Fix: the grids were widened. Tweedie now selects α = 10 in 5/5 folds and nothing sits at
     an edge; the report records the boundary flags.
   - The same principle as `GridBoundaryError`: a boundary choice belongs to the grid, not the
     data.
3. **The OOF feature file's `labels` are placeholder zeros**
   (`research/v3/extract_oof_features.py:149`), so alignment is now checked on image IDs.
4. ⚠️ **Still open:** A3's CV bandwidth sits at the edge of the widened grid (h = 100). CV cost is
   still falling as locality disappears, meaning the data prefer no age locality below 60.
   Reported as a finding, not refit.

**Standing checks.**
- The `frozen_params` self-test passes.
- `results/test_pass_receipt.json` is still at `n_executions: 2`.
- The S54 and S56 reserved receipts are unchanged.
- **Ledger.** `research/experiments.csv` gained 20 `v4_s57a` rows: 8 arms × {oof, val}, 1 audit
  row and 3 estimator rows. They were verified against a pre-session copy: append-only, with no
  duplicate (method, split). Both runners prune their own rows, and a re-run replaced them
  cleanly.

### S64 — under-40 decision-rule ceiling, and Phase X added to the V4 runbook (2026-09-17)

No test read, no reserved read, no GPU. Asked after S57a: can a dynamic λ still rescue under-40, and
would pooling all three datasets with ensembling, TTA, calibration and referral do it?

**Ceiling** (`research/v4/ceiling_u40.py`, `--selftest` 8/8; `results/v4/s64/ceiling.json`,
`frontier_oof.csv`, `paper/figures/under40_ceiling.png`). HAM OOF, S57a's panel, score
`d = p_maxE − p_maxN`, so every constant λ is one threshold on d.
- Under-40 AUC **0.878 [0.811, 0.942]** vs 40-59 **0.948** and 60+ **0.929**. At specificity 0.90,
  under-40 sensitivity is **0.672** vs 0.861 / 0.759.
- The frozen rule is already on the frontier: 0.625 at under-40 referral 0.076.
  - A constant λ needs referral **0.259** for sensitivity 0.80 and **0.401** for 0.90.
  - The in-sample per-5-year-bin oracle, which bounds every λ(age), needs **0.168** and **0.253**. That bound is optimistic: 64 positives over 8 bins, and S57a's nested CV showed the gain does not hold out.
- Ensembling does not move under-40 ranking: soft-vote vs best single member (ConvNeXt-Tiny, picked in-sample) **+0.0037 [−0.0421, +0.0516]**. The Dirichlet map adds +0.0072.
- Binormal sizing: sensitivity 0.80 at specificity 0.90 needs under-40 AUC **≈ 0.93**, so S67's target is **+0.05**. The binormal fit under-predicts at specificity 0.95 (0.50 vs empirical 0.625) and is close at 0.90 and 0.85.
- 4 ledger rows `session=v4_s64`, verified append-only.

**Pooling answered from existing results.** V4's pooled train split already has **81** under-40 escalating lesions (HAM 34, BCN 35, MSKCC 12); counts are from `ml/data/manifest_v4.csv`. S54 trained on it, and under-40 pAUC stayed flat. An ensemble of the six V4 models is untested; it is S67 stage 0.

**Runbook** (`V4_SESSION_RUNBOOK.md` §6b, Phase X). Numbered S64–S67 so nothing is renumbered; they run between S57b and S59.
- S64: the ceiling, done.
- S65: combined per-band triage policy (S55 + λ + S56).
- S66: per-hospital λ.
  - The V1 arm needs no GPU: S13's frozen predictions cover the non-reserved BCN (117 images / 35 lesions) and MSKCC (12 / 12) under-40 escalating rows.
  - MSKCC fails the 30-positive gate, so the primary arm is hierarchical shrinkage.
- S67: staged under-40 ranking programme; the GPU runs only if a CPU probe passes.
- Session table, GPU index, dependency map and expected-value table updated. The GPU time quoted for the optional S66 V4 arm is labelled as an extrapolation.
- The runbook already had an S57c (gated λ(age, margin)). The ceiling was first run under that ID; it was renamed to S64 and re-run, and its first ledger rows were removed by restoring the pre-session ledger copy.

### S57b — λ(age) cross-fitting, stability, gates and freeze decision: REJECT-cost (2026-09-17)

No test read (receipt still `n_executions: 2`). Reserved read **once**
(`results/v4/s57b/reserved_receipt.json`: 1 execution, completed, no rerun reason).

**Built.** Self-tests 8/8, 6/6 and 9/9 respectively.
- `research/v4/lambda_crossfit.py` — phase 4, the development ablation and the deviance test.
- `lambda_stability.py` — phase 7 and Gate 5.
- `lambda_freeze.py` — the plan, the read, the verdict, and the freeze (on ADOPT only).
- `lambda_transport.py` — runs after a freeze only, so it refuses to run on this verdict.

**Phase 4, fully nested cross-fit** (`results/v4/lambda_crossfit_audit.json`,
`results/v4/s57b/{crossfit_rows,ablation_dev}.csv`).
- Per outer fold, the inner-cross-fitted calibrator, hyperparameters (S57a's nested CV re-run inside
  the fold), λ and an A1 refit never see the scored fold. This is asserted, and no lesion crosses.
- **Hyperparameters move:** κ is 0.1 in folds 0–1 and 0.001 in folds 2–4. Bandwidth 100 and τ = ∞
  are the same in every fold.
- Cross-fitted cost: A1 **0.398** stays best (A2 0.431, A3 0.406, A6 0.412, A7 0.417).
  Under-40 sensitivity / referral: A1 0.609 / 0.073, A3 0.703 / 0.149, A7 0.641 / 0.121.
- The declared dev-best rule picked **A3**, so the confirmatory family is {A3, A7}.
- **Deviance diagnostic:** three band steps vs + spline(age), LR **35.6** on 7 df, p **8.6e-6**.
  Under 40, linear age p 5.4e-5. Age carries structure beyond three steps; whether it clears the
  clinical bar is a separate question.

**Phase 7, 400 lesion-grouped bootstrap refits** (`results/v4/lambda_bootstrap.json`,
`lambda_curve.{csv,json}`, `s57b/bootstrap_tables.npz`, `paper/figures/lambda_age_curve.png`).
- The full-data refit reproduced every S57a table exactly (max |diff| 0).
- Gate 5 A3 **passes**: band width 0.134 < distance to A1 0.198, sign agreement 0.97.
- Gate 5 A7 **fails**: band width 0.372 > distance 0.098.
- A3 is stable because it is nearly flat (≈0.66 below 55, ≈0.33 above).

**Plan** `results/v4/s57b_plan.json`, sha256 `e00aafd837e62d74…`.
- Written before the read; every input is hashed, and the read refuses to run on any drift.
- Gates:
  - G1: Δ ≥ 0.10, lower bound > 0 and Holm p < 0.05.
  - G2: development CP upper bound ≥ 0.85 per band, and reserved specificity ≥ A1 − 0.02 per band.
  - G3: Macro-F1 and balanced-accuracy lower bound > −0.02, and older-band sensitivity ≥ A1's CP lower bound.
  - G4: blowout if under-40 referral ratio > 1.5.
  - G5: as above.
- Paired lesion-grouped bootstrap, 2000 resamples, seed 42.
- A smoke run on HAM val rehearsed the whole path first (A1 Macro-F1 0.7713 reproduced).

**Reserved result** (`results/v4/lambda_verdict.json`, `results/v4/lambda_ablation.csv`,
`paper/tables/lambda_ablation.tex`).
- A1: under-40 sensitivity 0.409 at referral 0.145; Macro-F1 0.4452; specificity 0.960 / 0.748 / 0.827.
- **A3 → REJECT-cost.**
  - Gate 1 passes: **+0.208 [+0.127, +0.296]**, Holm p 0.002.
  - Gate 4 blowout: under-40 referral 0.145→**0.278**, ratio 1.92; ΔNNB(π 0.03) under 40 +4.3.
  - Gate 2 fails: under-40 specificity 0.960→0.857.
  - Gate 3 passes: Macro-F1 +0.0035 [−0.0048, +0.0126].
- **A7 → REJECT-unstable.**
  - Gate 1: +0.068 [+0.030, +0.115], below the MCID.
  - Gate 5 fails.
- **Overall: REJECT-cost.** Nothing frozen, no `analysis_plan_s57.json`, A1 stays deployed, **S57c skipped**.
- Exploratory steps on reserved:
  - A0→A1: +0.068 under-40 sensitivity.
  - A1→A2: +0.104, driven by the pooled-λ fallback in 7 of the 8 under-40 bins.
  - A2→A7: −0.036.
- **Reading:** A3's gain is a larger under-40 λ (≈0.66 vs 0.26) on a cohort whose under-40 optimum is
  higher than HAM's (S14: BCN 0.91). It is the ROC trade S64 measured, and a per-centre λ question for S66.
- `sklearn` warned that `y_pred contains classes not in y_true` in the balanced-accuracy bootstrap
  (a class absent from a resample). The warning is harmless; the balanced accuracy computed is still valid.

**Records.**
- Ledger: 19 `v4_s57b` rows, append-only against a pre-session copy, 0 duplicates.
- `frozen_params` self-test passes, and `research/agerule/` is unchanged against HEAD.
- Runbook: S57b marked REJECT-cost and S57c marked skipped.

### S58 — domain router + domain-matched head: head SUPPORTED, routing and the gate do not earn their place (2026-09-17)

No HAM test read. The reserved cohort was read **once**; receipt
`results/v4/s58/reserved_receipt.json` has 1 execution and no rerun reason. The plan
`results/v4/s58_plan.json` has sha256 **`cc4b205a2b60f1dc…`**, frozen after val selection and
before the read. The runner is `research/v4/s58_front_end.py` (self-test **14/14**). Results are in
`results/v4/s58/` (`s58_report.json`, `stage1_gate.csv`, `stage3_marginals.csv`,
`stage3_contrasts.csv`).

**Design, frozen before the read.**
- **Trunk.** One frozen `convnext_tiny_best.HAM-only.pt`, penultimate features, 224 px, 1 view.
- **Gate.** `research.selective.mahalanobis` fit on V4 train, cut at the held-out in-scope 95th
  percentile.
- **Router.** Multinomial LR over ham / bcn20000 / mskcc.
- **Heads.** Balanced L2 LR heads fit on CPU (a declared deviation from the runbook's "GPU head";
  the model class is the same), with C chosen on val + ham_val. The C grid was widened to 1e-4…3
  after the first fit sat on its lower edge; `--select` now refuses a boundary C.
- **Arms.**
  - H0: checkpoint layer.
  - H1: pooled head.
  - **H2: routed.**
  - H3: oracle-routed.
  - D1: routed with a 3-class MSKCC head.
- **Primary.** Macro-F1 H2 − H0, MCID 0.020; Holm over {Macro-F1, under-40 pAUC@0.20}.
- **Design rule.** H2 must be non-inferior to H1 (margin 0.005), or the pooled head becomes
  stage 3.
- **MSKCC eligibility.** MSKCC holds only `bkl`/`mel`/`nv` in every split, so it fails the
  "every class ≥ 20 train images" rule and routes to the pooled head.
- **Leakage.** The v3 BCN/MSKCC holdout caches overlap reserved (**1,135 + 262** images), so all
  features were extracted fresh. Fit rows (19,096) share 0 images and 0 groups with reserved.

⚠️ **Transform defect caught before the freeze.** The first extraction reused S51's
`extract_backbone_features._eval_transform`, which resizes **bicubic**. The deployed
`ml.preprocessing.transforms.build_eval_transform` resizes **bilinear**.
- The checkpoint layer was exact: applied to the old cache it matches the published logits to
  1e-6.
- The ISIC-2019 HAM JPEGs are byte-identical to HAM10000's.
- Even so, the bicubic features sat **22% (relative L2)** from the deployed ones. H0 agreed with
  the published argmax on only **95.5%** of HAM val, and its Macro-F1 dropped **0.748 → 0.727**.

A refitted head would have recovered that interpolation penalty and booked it as head gain. Every
S58 cache was re-extracted with the deployed transform, reserved included (features only; no labels
are read at extraction). H0 now reproduces the checkpoint on **100.00%** of HAM val, and
`--select` refuses below 99.9%.
- ⚠️ **S51's ConvNeXt control also ran bicubic.** Its under-40 pAUC was 0.7319; H0 under the
  deployed transform gives **0.7110** on the same reserved rows. S51 compared all three backbones
  under their own bicubic pipelines, so its verdict is not obviously reversed. But its control
  figure is not the deployed model's, and should not be quoted as such.

**GPU (run in-session at the user's explicit request).**
- Two workers were refused by the guard: commit headroom was **6.02 GB** against the **6.9 GB**
  needed. The run used 1 worker.
- The smoke run caught a Windows spawn-pickling bug (a `PadDataset` class defined inside a
  function).
- Earlier, a CPU rehearsal on synthetic features caught three more bugs: pandas ≥ 2.2
  `groupby().apply` dropping the grouping column, `searchsorted` on an unsorted tuple, and an
  empty bootstrap crashing.
- Measured: 19,096 images in **2.8 min**, PAD's 2,106 in 0.8 min, reserved's 4,733 in 0.9 min.

**Val selection** (`results/v4/s58/selection_val.json`). H0 0.5519, H1 **0.6103**, H2 0.5969,
H3 0.6030, D1 0.6002; router accuracy 0.9198. Pooled already led on val.

**Reserved results.**
- **Primary — SUPPORTED.** H2 − H0 Macro-F1 **+0.1071 [+0.0749, +0.1360]**, Holm p **0.002**. H0 is
  0.4188 and H2 is 0.5259. The same move is significant on the gate-accepted subset:
  +0.0902 [+0.0512, +0.1220].
- **Under-40 ranking — not moved.** pAUC H2 − H0 **+0.0071 [−0.0271, +0.0400]**, Holm p 0.725.
  - Every arm and reference sits at 0.711–0.734 (V1 0.7335). This is the S54/S64 ranking limit
    again.
  - Under-40 escalation sensitivity at argmax does rise (H0 0.470 → H1 0.634; V1 0.341). That is
    an operating-point shift, not better ranking.
- **Design rule — routing loses.** H2 − H1 **−0.0081 [−0.0241, +0.0073]**. The lower bound is
  below −0.005, so **stage 3 = pooled head H1** (0.5340 [0.4862, 0.5715]). The router is kept
  only as a provenance / drift signal.
  - The BCN-matched head does not beat the pooled head on BCN itself (0.5201 vs 0.5267).
  - Router cost (H3 − H2) is small: +0.0052 [+0.0002, +0.0118].
- **D1 (not deployable).** The label-restricted MSKCC head adds +0.0040 [−0.0090, +0.0131]
  overall, and on MSKCC 0.2838 → 0.3047. MSKCC's 7-class Macro-F1 cannot exceed 3/7 by
  construction.
- **How much a head recovers.** The pooled head moves Macro-F1 0.419 → 0.534, about **63%** of
  the way to the retrained V4 pooled control (0.588 / 0.609 / 0.611 for seeds 42/43/44; `_last`).
  S42's "the head is mis-aimed" holds within dermoscopy, but a head alone does not match
  retraining.
- **Gate — fails its PAD target.**
  - In-scope false-reject on reserved is **0.067 [0.056, 0.077]** (target ≤ 0.10, met).
  - PAD reject is **0.378** against a target ≥ 0.80. AUROC reserved-vs-PAD is **0.799**, and
    ham_val-vs-PAD is 0.894, close to S8b's 0.913.
  - Fitting the gate on the pooled, heterogeneous corpus widens the in-scope region until most
    smartphone photos fall inside it.
  - ⚠️ The gate rejects **escalating** lesions more often: **0.087 [0.069, 0.104]** against 0.067
    overall.
  - **As built, the gate is not a usable admissibility stage.**
- **Router — misses its target.** Accuracy **0.932** against ≥ 0.95. BCN 0.948, MSKCC 0.862;
  within `nv` 0.902, within `mel` 0.931. The S43/S49 archive AUC of 0.990 was a pairwise ranking,
  not a 3-way decision accuracy.

**For S59.**
- Compose with the pooled head H1 as stage 3.
- Smartphone rejection needs a different gate: for example, fit on HAM only, as in S8b, or use a
  dedicated modality classifier. That is new work, not a threshold tweak.
- S66's per-centre λ must account for ~7% mis-routing if it uses this router.

### S66 — λ per hospital (per-centre escalation bias, V1 arm): REJECT-cost (2026-09-17)

**What.** `research/v4/s66_lambda_centre.py` (`--selftest` 14/14 once the plan exists) fits one λ per
(centre, age band) for the frozen V1 ensemble, using data that never touched reserved:
- HAM: the S5 cross-fitted OOF rows (6,981).
- BCN and MSKCC: their V4 **train** rows scored by S13's frozen V1 files. BCN has 6,478 rows,
  with 117 under-40 escalating images from 35 lesions. MSKCC has 1,609 rows, with 12 images from
  12 lesions.

V1 never trained on BCN or MSKCC, so no GPU was needed. The V4 arm was not built: the runbook
makes it optional, and needed only if S65 adopts a V4 base.

The objective is S5's: expected cost, with escalation specificity ≥ 0.85 in each band. The fits
reuse S57a's machinery unchanged (`decision`, `Scorer`, `repair_floor`, `nested_cv_panels`).

**Arms.**
- **P0:** the frozen HAM λ everywhere.
- **PC:** one λ per band, pooled over the three centres.
- **P1:** independent per-centre λ, falling back to PC below 30 positives.
- **P2 (primary):** hierarchical, α = P/(P+τ).
- **P2r:** P2 applied with the centre S58's router predicts.

**Why.** S14's target-fitted under-40 λ ranged from 0.12 to 0.91 across centres. The runbook
asked whether per-centre fitting turns that spread into a gain.

**Declared deviations (D1–D7, in the plan).**
- **D1:** τ is chosen by lesion-grouped 5-fold CV *within* each centre, not leave-one-centre-out.
  With a centre held out there is no local estimate, so every τ collapses to PC.
- **D2:** HAM folds use S57a's nested panels.
- **D3:** α counts images, not lesions.
- **D4:** `scc` rows are not fitting data; reserved includes S54's 146 top-ups.
- **D5:** V4 val has 0 under-40 escalating images, so it is descriptive only.
- **D6:** V1 probabilities only. S58's pooled head was trained on these same rows.
- **D7:** the dev ablation is cross-fitted at τ\*, which was chosen on the same folds.

**Development results** (`results/v4/s66/lambda_by_centre.json`, `ablation_dev.csv`), all stated
in the plan before the read:
- **τ\* = ∞.** The CV grid is flat within 1 SE (minimum at τ = 100), so the 1-SE rule takes full
  shrinkage. P2 therefore equals PC except where the per-centre floor repair lowers a band: BCN
  40-59 −0.11 and 60+ −0.06, MSKCC 40-59 −0.16. **The data do not support a per-centre deviation
  beyond what the floor forces.**
- **HAM's local band λs reproduce S5 exactly** (0.26 / 0.74 / 0.33, pooled 0.65; asserted).
- **BCN's under-40 λ is capped by the floor, not set by cost.** Its local value is **0.66**, where
  in-sample specificity is 0.852. S14's 0.91 had no floor.
- **The pooled under-40 λ is also 0.66**, driven by BCN's 117 of the 193 pooled positives.
- **P0 has the lowest CV cost (1.218 vs P2 1.296), but only because it breaks the floor.** Its
  cross-fitted BCN 40-59 specificity is ~0.75. Cost comparisons between arms that are held to the
  floor and one that is not are not like-for-like.
- **P2 also moves HAM.** HAM's under-40 λ goes 0.26 → 0.66, and its cross-fitted under-40
  referral goes 0.076 → 0.153. HAM has no reserved rows, so this is a dev-only cost S65 must weigh.

**Plan.** `results/v4/s66_plan.json`, sha256 **`caf910c5a71962dd…`**; 12 inputs hashed, including
S58's router state and reserved features.
- **Primary endpoint:** BCN reserved under-40 sensitivity, P2 − P0. Lesion-grouped (`group_id`)
  paired bootstrap, 2000 resamples, seed 42. A single-member family.
- **Gates:** S57b's, kept unchanged: MCID 0.10; specificity non-inferiority margin 0.02;
  Macro-F1/BA non-inferiority margin 0.02; older-band sensitivity ≥ P0's Clopper–Pearson lower
  bound; under-40 referral-ratio blowout at 1.5×.
- **G5 (new):** the gain must survive routing (P2r − P0 lower bound > 0).
- **Stated in advance:** gate 4 was expected to fire (dev ratio 3.2). P0's BCN under-40 referral
  (0.062) is below BCN's under-40 prevalence (0.119), so a ratio rule is harsh there. Extra
  referrals per extra catch are reported beside it as a descriptive figure.

**Reserved results** — read **once**; receipt `results/v4/s66/reserved_receipt.json` shows 1
execution, no rerun reason, verdict REJECT-cost. Outputs: `results/v4/s66/s66_report.json` and
`reserved_arms.csv`; 13 ledger rows under `v4_s66` / `S66_reserved_*`. The smoke run (BCN/MSKCC
dev rows, in-sample) is in `results/v4/s66/smoke/`.
- **Verdict: REJECT-cost.** G1 passes; G2, G3 and G4 fail; G5 passes. **P0 stays deployed, and
  S65 uses the frozen 3-band rule.**
- **G1 (primary, BCN under-40 sensitivity).** P2 − P0 = **+0.216 [+0.125, +0.313]**, bootstrap p
  0.001 (the resolution floor). Sensitivity goes 0.400 [0.339, 0.463] → **0.616** [0.553, 0.676];
  missed cases 153 → 98 of 255 images.
- **G4 (referral, BCN under-40).** Referral goes 0.160 → **0.303**, ratio **1.90**, Δ +0.143
  [+0.106, +0.187]. That is 107 extra referrals for 55 extra catches, **1.95 per extra catch**
  (P0 − A0: 1.5). Under-40 NNB at π = 0.03 goes 3.8 → 8.4.
- **G2 (specificity).** Under 40, specificity falls 0.965 → 0.859, so G2 fails. P2 does repair
  P0's older-band floor breaks: 40-59 0.757 → **0.862**, 60+ 0.819 → 0.842. 60+ is still below
  0.85 on reserved, so the floor does not transfer (as in S57a).
- **G3 (non-inferiority).** Macro-F1 −0.008 [−0.021, +0.004] fails its −0.02 bound. Older-band
  sensitivity also falls below P0's lower bound: 40-59 0.721 → 0.641, 60+ 0.568 → 0.526. That is
  the price of the floor repair.
- ⚠️ **The gain does not come from per-centre fitting.** PC − P0 is the same **+0.216**, and
  P2 − PC is **exactly 0.000** under 40. All the under-40 gain is the pooled λ (0.66 vs 0.26),
  and per-centre repair only costs Macro-F1: P2 − PC −0.0087 [−0.0188, −0.0011]. This is the S57b
  A3 finding again (a bigger under-40 λ), now on BCN.
- **G5 (routing) passes.** P2r − P0 = +0.216 [+0.129, +0.312]. Router accuracy on reserved is BCN
  0.948 and MSKCC 0.862 (S58's numbers reproduced). Every mis-routed under-40 row lands on a table
  with the same under-40 λ, so mis-routing is nearly free *because P2 has no per-centre under-40
  structure*.
- **MSKCC (descriptive, 24 under-40 escalating images).** Sensitivity 0.500 → 0.625, +0.125
  [+0.000, +0.278]. Referral 0.099 → 0.198: 23 extra referrals for 3 extra catches (7.7 each).
- **P0 − A0 on BCN** (the frozen rule's own transfer): under-40 sensitivity +0.071
  [+0.037, +0.108], Macro-F1 +0.042 [+0.020, +0.066]. This is consistent with S14.

**What it settles.** Per-centre λ adds nothing the data can support: τ\* = ∞, and P2 − PC = 0
under 40. The available BCN under-40 gain is the S57b pattern: raise under-40 λ and pay about 2
referrals per extra catch, with older-band sensitivity lost to the floor. The runbook's caveat
held: this compensates for V1's poor BCN transfer, and does not fix ranking (S64). Nothing is
frozen, and `frozen_params.load_lambda_by_centre()` is **not** added.

**For S65.** Take the frozen 3-band rule. If S65 wants a larger under-40 λ, the reserved cost is
now measured twice: S57b on all of reserved, S66 on BCN alone.

### S65 — combined per-band triage policy (S55 + frozen λ + S56): REJECT-cost (2026-09-17)

**What.** `research/v4/s65_combined_policy.py` (`--selftest` 7/7) composes the three decision-layer
tools on the frozen V1 ensemble, in S59's order:
1. S55 per-band Dirichlet (`frozen_params.calibrate_by_band`; cross-fitted within band on OOF).
2. The frozen 3-band λ rule (`frozen_params.apply_age_rule`), **not refit** (S57b and S66 both
   left it deployed).
3. S56's per-band CRC-floor abstention, **refit** on the per-band-calibrated OOF scores, with the
   λ decision defining which escalating rows are still silent misses.

Fit on OOF, score selected on val by S56's own rule (**entropy**; msp/margin/entropy eligible),
reserved read **once** (`results/v4/s65/reserved_receipt.json`, 1 execution, no rerun reason).
No HAM test read, no image scored. Plan `results/v4/s65_plan.json`, sha256
**`22c964fdb5810ab6…`**. Outputs: `results/v4/s65/{selection_val.json, policy_oof.json,
frontier_{oof,val,reserved}.csv, contrasts_reserved.csv, s65_report.json}`, val rehearsal in
`results/v4/s65/smoke/`. Ledger `v4_s65`: 29 val rows + 1 plan + 30 reserved rows (pruned on
re-run).

**Arms (all paired on the same rows).** `S56` (global map, argmax, S56's msp policy),
`LAM` (global map + λ, no referral), **`COMB`**, `COMB_global` (one threshold),
decomposition arms `CAL_S56` / `LAM_S56`, and **`S56_matched`**. λ escalations fall outside the
abstention budget R, so the plan declared a workload-matched comparator: S56 at the smallest
budget whose OOF flag rate (escalated or referred) reaches COMB's at R = 0.20 → **R = 0.305**
(target 0.3823). The verdict rule, frozen before the read, required that contrast to be
positive for ADOPT.

**Integrity.** The `S56` arm reproduces S56's published frontiers exactly — val drift 1.1e-16,
OOF in-sample exact, reserved 8.3e-17 (the run refuses otherwise).
`research/v4/s56_abstention.py` gained an optional `Panel.pred` and a `Panel.score()` helper so
its fitter accepts a non-argmax decision; no S56 behaviour changed (selftest 9/9, the
reproduction above).

**Pre-read finding (OOF/val, written into the plan).** ⚠️ **The per-band map undoes most of the
frozen under-40 λ**, which was fit under the global map. Under-40 decision sensitivity LAM →
COMB_none: OOF 0.625 → **0.438**, val 0.591 → **0.273**, reserved 0.409 → **0.262**
(`frontier_*.csv`). COMB then buys its under-40 floor with abstention: OOF under-40 referral
**0.608** at R = 0.20 vs S56's 0.317.

**Reserved results** (`results/v4/s65/s65_report.json`, `contrasts_reserved.csv`; R = 0.20):
- **Primary SUPPORTED.** Under-40 system sensitivity COMB − S56 **+0.082 [+0.042, +0.131]**
  (0.846 vs 0.763), MCID 0.05.
- **Non-inferiority on 60+ passes.** COMB − LAM **+0.211 [+0.179, +0.243]**, margin −0.05. The
  comparator has no referral, so this is easy to pass. The S56-style companion,
  COMB − COMB_global on 60+, is **−0.092 [−0.112, −0.074]**: the same cost S56 measured
  (−0.100).
- ⚠️ **Workload-matched: null.** COMB − S56_matched under 40 is **+0.004 [−0.014, +0.025]**.
  The budgets were matched on OOF. On reserved, COMB's all-ages flag rate is 0.038 lower than
  S56_matched's (0.550 vs 0.588), so the two arms are not exactly matched.
  - What S56_matched does with its extra flags: all-ages sensitivity +0.037 over COMB
    (0.794 vs 0.757) and 60+ +0.024.
  - Under 40, S56_matched matches COMB's sensitivity with fewer flags (0.558 vs 0.608) and keeps
    under-40 retained Macro-F1 at 0.409 (COMB 0.134).
  - So S56 alone is at least as good wherever the two are comparable. *(Wording made precise in
    the final audit; the verdict does not change.)*
  → **Verdict `REJECT-cost`**, by the pre-declared rule.
- **Cost table.** Under-40 referral **0.454 → 0.608** (+0.154 [+0.129, +0.181]) and under-40
  retained Macro-F1 **0.459 → 0.134**. All-ages referral 0.386 → 0.372, all-ages flag rate
  0.479 → 0.550 (+0.070), all-ages system sensitivity 0.693 → 0.757, retained Macro-F1
  0.465 → 0.501. 40-59 sensitivity −0.012 [−0.030, +0.005], 60+ +0.092. NNB(π = 0.03) under 40:
  15.7 → 20.6.
- **Decomposition (under 40).** Calibration alone: CAL_S56 − S56 **−0.007 [−0.022, +0.007]**
  (S64's "ranking-neutral" holds). λ alone: LAM_S56 − S56 **+0.050 [+0.023, +0.084]**.
  Calibration given λ: +0.032 [+0.009, +0.061], via the extra under-40 referrals above.
- **Floors stay nominal.** Decision specificity is 0.748 (LAM) / 0.759 (COMB) at 40-59 and
  0.827 / 0.788 at 60+, all below 0.85 (S57a replicated; per-band calibration makes 60+ worse).
  Realised all-ages referral is 0.372 at a nominal 0.20.

**What it settles.** The combined policy does lift under-40 system sensitivity over S56, but only
by spending more workload. At matched workload it adds nothing, so it is the S57b/S66 pattern a
third time. Composing S55 with a λ fitted under the global map is not neutral: per-band
calibration absorbs the under-40 base rate and the λ bonus with it. **Nothing is adopted.** S59
should compose S56 alone (global map, argmax) and choose its operating budget directly. If a
future session wants S55 in the stack, λ must be refit on per-band-calibrated OOF — a new tool
with its own plan, not a tweak to this one.

### S67 stage 1 — under-40 ranking probes on frozen features: STAGE2_NO_GO (2026-09-17)

**What.** `research/v4/s67_ranking_probe.py` (`--selftest` 5/5) is the CPU gate that stands before
any S67 GPU fine-tune. It asks whether changing a head's **objective or data weighting** moves
under-40 escalation pAUC@0.20. The trunk is frozen and unchanged (runbook §9 rule 6).
- **Features:** `research/v4/features/convnext_tiny_s58_fit.npz`, the HAM-only ConvNeXt-Tiny
  under the deployed bilinear transform (S58).
- **Development rows:** 12,115 rows the trunk never trained on — BCN and MSKCC `train` + `val`,
  plus HAM `ham_val`. HAM `train` is excluded because it is in-sample for the trunk. Under 40
  there are 2,012 rows, with **151 escalating images from 57 lesions**.
- **Reads:** no reserved or test row was touched. Stage 0 (the V4 ensemble on reserved) was
  **not** run: it is a reserved read and waits for the owner.
- **Cross-fitting:** `StratifiedGroupKFold` on `group_id`, 5 folds × 3 repeats, with scores
  averaged over repeats.
- **No tuning:** every C (S58's 0.001), weight and quantile is fixed in the plan.
- **Plan:** `results/v4/s67/stage1_plan.json`, sha256 **`20405144a9884445…`**, frozen before any
  fit.
- **Launch:** via WMI, outside the app container (parent WmiPrvSE). It ran in minutes on CPU.
- **Outputs:** `results/v4/s67/{probes.json, stage1_scores.csv, stage1.log}`, plus 7 ledger rows
  under `v4_s67`.

**Gate.** An arm passes if its under-40 pAUC minus the control's is ≥ +0.03 **and** its
lesion-grouped paired bootstrap interval excludes zero. The interval is taken at 1 − 0.05/3
because there are three arms; this is a declared tightening of the runbook's 95% interval.

**Results** (`results/v4/s67/probes.json`). The control is a 7-way pooled logistic head with
under-40 pAUC **0.728** (AUC 0.818).

| arm | under-40 pAUC | Δ vs control [98.3% CI] | all-ages Δ pAUC [95%] | gate |
|---|---:|---:|---:|---|
| A_specialist (under-40-only binary head) | 0.689 | **−0.039** [−0.108, +0.034] | −0.082 [−0.096, −0.069] | FAIL |
| B_reweight (×5 under-40 escalating, ×3 `nv` look-alikes) | 0.728 | **−0.000** [−0.013, +0.012] | −0.004 [−0.008, +0.001] | FAIL |
| C_metadata (+ sex, site, age) | 0.735 | **+0.007** [−0.002, +0.017] | +0.011 [+0.007, +0.015] | FAIL |
| H0 (checkpoint layer, descriptive) | 0.678 | −0.050 [−0.109, +0.015] | — | — |

- **No arm comes near +0.03.** The best, metadata, gains +0.007, and its interval crosses zero
  even at 95% ([−0.0001, +0.0153]).
- **The specialist is worse, not better.** With 57 lesions it has too little data, and it loses
  0.08 all-ages pAUC.
- **Reweighting is exactly neutral**, so weighting hard cases does not change the ranking a
  linear head can extract.
- **Metadata adds a small, certified all-ages gain** (+0.011) that does **not** concentrate
  under 40. It matches R7's age-flip span of 0.0475 falling short of its MCID.
- **The per-archive under-40 split is descriptive only**, with 117 / 12 / 22 escalating images:
  BCN 0.725, MSKCC 0.605, HAM val 0.752 for the control.

**What it settles.** `STAGE2_NO_GO`: **no S67 GPU fine-tune is handed off.** The runbook stated
the meaning of a null across S64–S67 in advance, and it now holds on the head side:
- S51 (backbone), S54 (pooled training), S64 (every cutoff) and S67 stage 1 (head objective,
  weighting, metadata) all leave under-40 ranking where it was.
- The under-40 blind spot is a ranking limit of dermoscopy models trained on these archives, and
  no decision layer or head removes it.

This is the finding to write up. The only unrun piece of S67 is stage 0, a small descriptive
reserved read that the owner may choose to spend.

### V4 audit — are the gate failures real, and can the numbers be trusted? (2026-09-17)

No test read and no new reserved read. Everything was checked from frozen artefacts. Two
checkpoints were re-scored on V4 **val** on CPU, into `scratchpad/audit_v4/` (outside `results/`).

**Integrity layer — clean.**
- **Receipts and plans.** Every V4 reserved receipt (S54, S56, S57a, S57b, S58, S65, S66) has
  exactly one completed execution per stage and no rerun reason. `results/test_pass_receipt.json`
  is still at `n_executions: 2`. All 8 plan hashes match their files, and every plan's timestamp
  precedes its read. All 19 S54 item hashes match.
- **Splits.** `manifest_v4` has zero overlap between splits on `group_id`, `lesion_id` or
  `dup_cluster`. `ham_val` and `ham_test` are identical to `split_v1`.
- **Frozen artefacts.** The frozen checkpoints are 6/6 byte-identical, and the `frozen_params`
  selftest passes.
- **Ledger.** It has no conflicting duplicates. The 8 repeated `v4_s57a` methods are one row per
  split (dev and val), not re-runs.

**Headline numbers — all reproduce** with independent code (sklearn `roc_auc_score(max_fpr=0.2)`
and `f1_score`, not the runners' own metrics):
- **S54 Gate B, from `results/v4/s54/predictions/`:** `_last` ΔMacro-F1 −0.0121 and Δpauc
  +0.0089; `_best` +0.0316 and −0.0106. All exact.
- **S56, from `policies_oof.json` applied to the frozen V1 reserved probabilities:** under-40
  system sensitivity band 0.7634 vs global 0.6093. Exact.
- **S58, from `fit_state.npz` × reserved features:** H0 Macro-F1 0.4188 and under-40 pAUC 0.7110;
  pooled head 0.5340. Exact.
- **S66 A0/P0 BCN band figures, and S57b/S65 A1 under 40** (sensitivity 0.4086, referral
  0.1452): exact.
- **S67 stage 1 from `stage1_scores.csv`, and S64 from `ceiling.json`:** exact.
- **Inference path.** `R0_pooled_s44_best` re-scored on the full V4 val (2,270 rows) gives
  Macro-F1 **0.587906**, identical to its training log. `R1+R4_pooled_s43_best` at 384 px gives
  0.622807 vs 0.622246. The +5.6e-4 is CPU float32 vs GPU AMP (a couple of argmax flips), not a
  pipeline difference.
- **S51 preprocessing is correct:** CLIP mean/std for the DermLIP tower, the timm config for
  DINOv2, and an asserted-exhaustive BEiT→timm remap. Its control used the bicubic transform
  (known since S58). With the deployed control (0.7110), PanDerm − ConvNeXt is ≈ −0.015 instead
  of −0.036, which still falls far short of the +0.05 falsifier.

**⚠️ Defect 1 — `research/v4/colour.py:shades_of_grey` is wrong, so R2 and S49 are invalid.**
- **The fault.** The scale factor is `‖e‖ / e_c`; the standard von Kries normalisation is
  `‖e‖ / (√3·e_c)`. For a neutral illuminant this multiplies every pixel by **√3 ≈ 1.73**
  before clipping.
- **Measured on 60 training images:**

  | | mean brightness | pixels saturated (mean / median) |
  |---|---:|---:|
  | input | 149 | 0.4% / 0% |
  | repo function | **218** | **72% / 79%** |
  | correct formula | 150 | 0.4% / 0% |

- **R2's −0.102 val Macro-F1 (S53)** measured training on blown-out images, not colour
  constancy. The rung is **untested, not failed**.
- **S49's verdict is unsupported.** It read "colour constancy materially reduces archive
  decodability (0.990 → 0.952)", but destroying most of the image lowers decodability with no
  illuminant mechanism at all. Treat it as unsupported until re-run.
- **Not fixed here.** The fix changes the R2 recipe that `recipe_ladder_plan.json` froze, and a
  valid R2 needs a GPU re-run by the owner.

**⚠️ Defect 2 — early stopping made the S53 ladder, and the S54 models, under-trained.**
- **Why it bites.** `EARLY_STOPPING_PATIENCE = 8` on a cosine schedule stops runs while the
  learning rate is still high.
- **R6 ("EMA + 60 epochs") stopped at epoch 19 of 60** (LR 8.2e-5), so its long-schedule
  hypothesis was never tested. EMA, which lags, makes the stale counter fire sooner.
- **R0 stopped at 23/30.** Only R1 and R4 ran all 30 epochs, and both peaked at the end (epochs 25
  and 29).
- **Pooled S54 runs stopped at 14–22 epochs.** Their `_last` checkpoints are 8 epochs past a peak
  that was itself taken at mid-schedule LR.
- **The ladder was ranked on one seed's noisy max-over-epochs.** S53 measured about 0.02 of jitter
  on that max; R1's +0.0225 is barely above it.
- **What this does not change.** S54's Gate B is still a legitimate comparison of the two
  procedures *as run*. But "in-domain training does not fix under-40 ranking" is conditional on
  this truncated recipe.

**Design caveats (not bugs).**
- **Reserved has been read 7 times across V4.** Each read is once-per-question and receipted, but
  which later sessions to run was chosen after seeing earlier results. The reserved cohort is now a
  reused evaluation set, not a pristine holdout. Cross-session multiplicity is uncorrected; this
  matters most for the positives (S56, S58's routed-vs-checkpoint).
- **Several gates were close to unpassable by construction.**
  - S57b/S66's 1.5× under-40 referral-blowout gate: any λ increase that adds sensitivity adds
    referral. S66 declared in advance that it expected this gate to fire.
  - S51's +0.05 falsifier at 104 lesions, where the pAUC interval is about ±0.07.
  - These are negative results against demanding bars, not proof of zero effect.
- **The under-40 ranking gap is partly case mix.** Under-40 escalating lesions on OOF are 51 mel /
  12 bcc / 1 akiec. Restricted to melanoma vs benign, AUC is **0.873** under 40 vs 0.940 (40-59)
  and 0.909 (60+). The age gap survives but narrows (vs 60+: 0.044 → 0.036). The write-up should
  state the melanoma-only comparison beside the all-escalation one.

**Which results stand.**
- **Stand as computed:** S51, S54 (with the Defect 2 caveat), S55, S56, S57a/b, S58, S64, S65,
  S66, S67 stage 1. The decision-layer results (S57b, S65, S66 REJECT-cost) do not depend on
  training, so Defect 2 does not touch them.
- **Invalid:** R2 and the S49 verdict.
- **Untested:** R6.
- **Weak:** the ladder ranking (R1's selection, R5's null).

**Next, owner's call.** Fix `shades_of_grey`, then re-run R2 and S49. Re-run R0 / R5 / R6 without
early stopping (or with patience ≥ the cosine tail). That is GPU work; measure it with
`scripts/gpu_benchmark.py` first.

### S53r — colour fix and re-run set-up (2026-09-17), GPU run pending (owner launches)

**What changed.**
- **`research/v4/colour.py`: the gain is now `‖e‖ / (√3·e_c)`.** It is the identity on a neutral
  image. On the same 60 training images, mean brightness is 149.3 → 149.3 with 0.4% of pixels
  clipped; the old version gave 218 and 72%. Two new `recipe.py` selftests guard it: 5.c2
  (neutral image unchanged) and 5.c3 (a tint is neutralised without saturating). `recipe.py
  --selftest` passes **39/39**.
- **`research/v4/train_v4.py` gained `--patience` and `--run-tag`.**
  - `--patience`: 0 disables early stopping; the default of 8 is unchanged.
  - `--run-tag`: suffixes the run id, checkpoints, run JSON and ledger method, so a re-run never
    overwrites its original. Tagged runs log to ledger session `v4_s53r` and record
    `early_stopping_patience`, `run_tag` and `rerun_plan_sha256`.
- **Superseded records (not deleted):**
  - S49's outputs were renamed to `results/v4/archive_probe_pre_post.superseded_colour_bug.{csv,json}`.
  - Their 6 ledger rows were retagged to session `v4_s49__superseded_colour_bug`.
  - The S53 R2 ledger row is now `R2_ham_only_s42__superseded_colour_bug`.
  - The original R2 run JSON and checkpoints are kept as history.

**Plan.** `results/v4/s53r_plan.json`, sha256 **`2ef2e91bf5db6df5…`**, frozen before any re-run
(`research/v4/ladder_rerun.py`, selftest 3/3). It hashes the colour fix and the unchanged ladder
plan.
- **Setup:** HAM-only screen corpus, patience off, seeds 42/43/44. R1 and R4 at seed 42 are
  reused because they ran their full 30 epochs; `--report` asserts that.
- **Primary reading:** final-epoch HAM val Macro-F1 minus R0, paired by seed. The reading is
  LEVER / NULL / HARM / MIXED at MCID 0.020; best-epoch is secondary.
- **Status:** a compute-allocation screen, not a test. No reserved or test read.
- **Schedule** (19 runs, estimates from each rung's own measured min/epoch × 1.05):

  | tier | runs | estimate |
  |---|---|---:|
  | A | R0 + R2 × 3 seeds | 4.2 h |
  | B | R6, R5, R7 × 3 seeds | 4.6 h |
  | C | R1, R4 × seeds 43/44 | 2.5 h |

  About 11.3 h in total, plus the S49 re-run and 7 smokes.

**Runner.** `scripts/run_s53r.ps1`:
- **Order:** S49 first, then one smoke per rung, then the schedule, then `ladder_rerun --report`.
- **Safeguards:** the shared GPU mutex, a 3-minute idle wait, a 6 GB disk floor, resume
  (banked runs are skipped), a 2→1→0 worker retry, a 20-minute stall watchdog (60 for S49), a
  `results/v4/s53r/STOP` file for a clean stop, and `status.json` every minute.
- **Dry run:** lists all 19 runs, 0 training processes, 26.3 GB free.
- **Not launched interactively** (runbook §7a).

**S53r runner hardened (same day).** `scripts/run_s53r.ps1` now survives a crashed night:
- **Restart and resume.**
  - An instance mutex keeps it to one copy.
  - On a manual launch it registers a Task Scheduler watchdog (`Capstone_S53r_Watchdog`) that
    relaunches it with `-Resume` every 15 min. `-Resume` is a no-op unless the run is `ARMED` and
    not `DONE`; this was verified.
  - The task removes itself when the run is `DONE`.
  - Banked runs and passed smokes are skipped on restart.
  - An orphaned training process is allowed to finish, and killed if it stalls.
- **Failure handling.**
  - A keep-awake request prevents sleep.
  - `--device cuda` makes a lost GPU fail fast, and the runner waits up to 30 min for it.
  - After the 2→1→0 worker retries, an out-of-memory error gets one more attempt at half batch ×
    double accumulation (the same effective batch).
  - Failed runs get a second pass.
- **Always a readout.** The report runs in a `finally` block, and `status.json` carries an ETA.
- **Checks run:** PowerShell parse clean, ASCII-only, dry run lists 19 runs (ETA 11.3 h), and
  task registration works without admin.
- **`ladder_rerun --selftest` 4/4:** a night with zero banked runs still yields a report.

### V4 findings draft + S59 build (re-scoped cascade, draft plan, val smoke) (2026-09-17)

No HAM test read, no reserved read, no ledger write. `research/experiments.csv` and
`results/test_pass_receipt.json` were sha256-checked before and after and are unchanged. Run
alongside a GPU job, so it was kept CPU-only: each step took under a minute.

**1. `paper/v4/v4_findings.md` (draft write-up).** New directory, no new computation. Every
number is cited to its `results/` file. It covers:
- the under-40 ranking limit (S51/S54/S58/S64/S67, with the audit's melanoma-only comparison);
- decision layers reallocating referrals (S56 positive with its cost; S57b/S65/S66 REJECT-cost);
- S55;
- the audit caveats (reserved read 7×, near-unpassable gates, the colour bug, early stopping).

The recipe-ladder section (§5) is a placeholder for S53r.
- **Recommended framing:** a *limits-and-accounting* paper instead of the runbook's
  "solutions paper". Of the four Part II mechanisms, only group-conditional abstention
  survived, and the Part III contract already fails to transfer (S56: 0/15 floors).
- ⚠️ **Hard Rule 4 gaps flagged in the draft:**
  - The melanoma-only AUCs (0.873 / 0.940 / 0.909) come from the audit's scratch computation.
  - So does the deployed-control PanDerm contrast (≈ −0.015).
  - Neither has a file in `results/`. Emit both on OOF before any manuscript use.

**2. S59 re-scoped: `research/v4/s59_cascade.py`** (`--selftest` 9/9).

*Stack.* Frozen V1, then a decision (argmax = `S56`, or the frozen 3-band λ = `CASC`), then S56
per-band CRC-floor abstention refit under that decision (score pinned to S56's `msp`), then an
optional conformal safety net.
- The conformal net is RAPS bipartite α=0.05 under the conformal Dirichlet map.
- Its 8 cell quantiles are **asserted equal** to
  `research/conformal/results_hierarchical_oof/fit_state.json`, and they reproduced exactly.

*Excluded, each with its reason written into the plan:*
- S58's gate and router;
- S58's head (different base);
- S55 (undoes λ, S65);
- λ(age) and per-centre λ (rejected).

*Adoption rule (S65's lesson).* A layer that adds flags outside the budget is adopted only if it
beats the stack without it **at matched OOF workload** by ≥ 0.01 val all-ages system
sensitivity.

*Deliverable.* The runbook's contract, issued on HAM and checked end-to-end on the evaluation
cohort:
- S = the OOF nominal floor, F = 1 − S;
- M = val retained Macro-F1;
- R = nominal budget + 0.05;
- N = val NNB at π = 0.03.

Each term is MET / NOT_MET / UNRESOLVED from lesion-grouped intervals, and the joint verdict is a
measurement, not a union bound. The declared expectation is **CONTRACT_FAILS** (S56, S57a).

**Selection** (`results/v4/s59/selection_val.json`):
- **Operating budgets** (smallest R with OOF S* ≥ 0.85): S56 **0.20** (S* 0.855), CASC
  **0.15** (S* 0.87).
- **λ layer not adopted.** CASC@0.15 − S56@0.235 (matched workload) on val all-ages is
  **+0.0032**, below the 0.01 margin. Under 40 it is +0.000.
  - At the *same* budget, CASC − S56 is +0.046 all-ages, which is purely a workload effect.
  - This is S65's pattern again, now on val.
- **Conformal layer not adopted.**
  - As a referral rule it raises the OOF flag rate to **0.854**: most benign-called cases have
    an escalating class in their set.
  - Its matched comparator is S56 at budget 0.83; val all-ages Δ is **−0.0032**.
  - Conformal sets stay an **output field** (S60 payload), not a decision layer.
- **Deployed arm: `S56@0.2`** — V1 + argmax + S56. Contract terms:
  - S = 0.855;
  - M = 0.888;
  - R = 0.20 (+0.05);
  - N = 7.92.

**Draft plan:** `results/v4/s59_plan.draft.json` (sha256 `501f65f51d03ee1f…`). It is **not
frozen**:
- `--freeze-plan` refuses without `results/v4/s53r/s53r_report.json` and `--confirm-base v1`.
- `--reserved` refuses without a frozen plan, and has its own receipt
  (`results/v4/s59/reserved_receipt.json`, not created).
- The plan records two open decisions for the owner:
  - the base model after S53r (a V4 base has no OOF predictions, so it would need a plan
    revision);
  - whether to add a target-side threshold recalibration arm on the BCN/MSKCC train rows that
    S66 used.

**Smoke on HAM val** (`--reserved --smoke --n-boot 500` → `results/v4/s59/smoke/`). The whole
path ran. Because M and N were issued on val, those terms are in-sample here.
- Coverage 0.795; joint verdict CONTRACT_UNRESOLVED.
- Under-40 sensitivity **0.909 [0.708, 0.989]**, 40-59 0.783 [0.643, 0.899], 60+ 0.889
  [0.824, 0.941]; referral 0.205, MET.
- Deployed − V1: all-ages system sensitivity **+0.156 [+0.108, +0.197]** at referral +0.205.
- Val holds 22 under-40 escalating images, so none of this decides anything (S48).

**Runbook:** the S59 heading now carries the re-scope note; the original six-stage text is kept.

### S53r readout → S59 freeze + reserved read → S60 service → S61 model card → S62 index (2026-09-17)

No HAM test read (receipt still `n_executions: 2`). One reserved read, S59's own (receipted once).

**S53r readout** (`results/v4/s53r/s53r_report.json`, run finished 16:24, 21/21 runs banked, plan
`2ef2e91b…`). Paired against R0 on HAM val over seeds 42/43/44: **R1 (384 px) +0.0297 Macro-F1,
seed range +0.0097 to +0.0466 (LEVER)**. The rest are NULL: R2 −0.0045 (−0.0224 to +0.0298),
R4 +0.0134, R5 −0.0112, R6 −0.0139, R7 +0.0058. The control's seed SD is 0.0230.
**R2's old −0.102 was the √3 colour bug.** With the bug fixed, R2 is a null result, not a failure.
None of this changes the base model: **V1 stays the base**.
- *Final-audit correction:* the first version gave the reason as "S54 Gate B found that R1+R4
  does not beat the V4 control". That argument only chooses between two V4 arms. It does not
  choose V1 over V4.
- Against V1, the V4 pooled control scores **+0.18 Macro-F1** on reserved (S54 Gate A,
  confounded by corpus).
- The actual reason is structural and is recorded in `s59_plan.json:base`: S56's thresholds are
  fit on cross-fitted OOF predictions. V1 has them; no V4 model does. A V4 base would need
  K-fold GPU training and a new plan.
- This is an open decision (see §V4 final audit), not a finding that V1 is better.

**S59 freeze and reserved read** (`research/v4/s59_cascade.py`; selftest 9/9;
`--freeze-plan --confirm-base v1` → `results/v4/s59_plan.json` sha256 **`e7ebc3f5…`**; then
`--reserved`, receipt `results/v4/s59/reserved_receipt.json`, execution 1, no rerun reason).
- **Deployed stack: S56@0.20 alone.** Neither extra layer beats S56 at matched workload.
  - λ layer, CASC@0.15 vs S56@0.235: −0.0179 [−0.0251, −0.0114] all ages, +0.000 under 40.
  - Conformal layer: −0.0692 [−0.0861, −0.0521] all ages, −0.1183 under 40.
  - Conformal sets stay an output field only.
- **Contract on reserved: CONTRACT_FAILS, every term NOT_MET.** This is the outcome the plan declared
  before the read.
  - `contract.coverage` = 0.614 is **selective coverage**: 1 − referral rate, the share of cases
    decided without referral (`s59_cascade.py:contract`).
  - The first version of this entry, the model card and the runbook read it as "61.4% of bootstrap
    resamples satisfy all terms together". That reading is impossible: every term's interval
    misses its target, so no resample satisfies them all.
  - Corrected in the final audit. The S60 payload field is renamed `joint_coverage` →
    `selective_coverage`.
  - Under-40 system sensitivity 0.763 [0.655, 0.853] against a floor of 0.855.
  - 40-59: 0.704 [0.631, 0.771]. 60+: 0.674 [0.638, 0.713].
  - Retained Macro-F1 0.465 [0.401, 0.516] against a target of 0.888.
  - Referral 0.386 [0.363, 0.408] against a limit of 0.250. NNB(π=0.03) 13.6 against 7.9.
- Compared with V1 at full coverage, the deployed stack raises system sensitivity by
  +0.301 [+0.273, +0.328] overall and +0.423 [+0.310, +0.539] under 40. It refers
  +0.386 [+0.363, +0.408] more cases.
- Conformal coverage on reserved is 0.854 / 0.825 / 0.779 by band against a nominal 0.95 (`s59_report.json:conformal`).
- **S63's test-read gate is closed**: the cascade does not clear its per-band floor on reserved, so
  there is no HAM test read.

**S60 — inference service** (`research/v4/s60_contract.py`, `research/v4/s60_api.py`,
`tests/test_s60_service.py`; selftest 11/11, pytest 19/19).
- **Endpoints.** FastAPI with `/health`, `/contract` and `/predict`.
- **Stubbed model.** The request carries the 7-class softmax vector. No model is loaded and there is
  no ONNX export: the base is the 6-CNN V1 ensemble with 24-view TTA, so export is packaging work.
- **The decision uses only frozen files.**
  - Deployed Dirichlet map, via `frozen_params.calibrate`.
  - Argmax class.
  - S56 `band_0.2` thresholds on `msp`, compared as `u >= min(tau_band, tau_global)`.
  - S6 RAPS bipartite α=0.05 set. k_reg=1 and λ=0.05 come from `session6_logs/S6_hierarchical.log`,
    because the fit_state does not store them. Scoring is non-randomised, hence conservative.
  - Per-band OOF NNB from `frontier_oof_insample.csv`.
  - The frozen band λ, reported but not applied.
- **Absent on purpose.** The OOD score and the Grad-CAM overlay are returned as null with a reason
  (both need the model). Age is never estimated: it is `recorded` or `unknown`.
- **Every response carries** the S56 plan hash, the S59 plan hash, the reserved `external_contract`
  (CONTRACT_FAILS), the under-40 ceiling notice, and the **mandatory Fitzpatrick notice**.
  The notice states the I–IV spread (0.102, non-monotonic) and that the system has
  **no evidence for types V/VI**.
- **Startup guard.** The service refuses to start if S59's report names a stack other than S56@0.20,
  or if `s59_plan.json` has changed.
- Correction to the first draft: `torch` *is* imported, CPU-only, via
  `research.calibration.methods`. The docstrings now say so.

**S61 — model card** (`paper/v4/model_card.md`).
- **Structure.** Built on CLAIM 2024 and TRIPOD+AI, with a mandatory fairness section and the S59
  contract table.
- **Drift hooks.** Four hook specifications: archive-probe, Mahalanobis (`research/selective/mahalanobis.py`),
  per-band coverage with S59's reserved reference values, and λ(age)-curve. None is wired to a
  running monitor yet; the card says so.
- **Fixed while drafting.** The Mahalanobis path had pointed at a module that does not exist.
  0.7871 had been labelled rung A5; it is rung A7-oof.
- **S56 floor transfer is 12 violated plus 3 below the floor within their CI**
  (`s56_report.json:floor_transfer`), not "10 violated".

**S62 — V4 index freeze** (`research/v4/s62_freeze.py`, `--check` PASS).
- **Output.** `results/v4/analysis_plan_v4.json` (sha256 **`bd320319…`**) indexes the 11 V4 gates:
  S51, S53r, S54, S56, S57b, S58, S59, S64, S65, S66, S67.
- **Per gate.** Each entry holds the plan hash, the report hash, the receipt's executions and rerun
  reasons, and the verdict read from the report.
- **Registration.** The index is registered under `analysis_plan_v4` in `results/frozen_artifacts.json`
  by a read-modify-write. The other keys (`analysis_plan`, `analysis_plan_v3`, and all 34 file hashes)
  were verified unchanged against a backup.
- **Families.** `research/stats/families.py` gains eight `v4_*` families, so no V4 multiplicity rule
  lives only in a plan JSON (the S14 gap).
- **Timing, stated plainly.** The index was written after the reads it lists. No λ(age) curve is
  frozen, because S57b rejected it.

**Verification.**
- `audit_v4 --check` 55/55. `audit_manuscript` 366/366.
- `frozen_params --selftest` passes; `s62_freeze --check` passes.

**Open.**
- `research/v4/audit_v4.py` (written by the parallel session) does not yet list the S59 receipt in
  `RECEIPTS`, and I left it alone because that session owns the file.
- S63 is now a writing task only: reframe the manuscript as a limits-and-accounting paper,
  with no test read.

### V4 final audit + S63 close-out (2026-09-17)

No HAM test read (receipt still `n_executions: 2`). No new reserved read. No GPU. Every number in
the V4 sections of this file (S48 → S62) was checked against its `results/` file. Where a
number had no file, the audit either created one from development data or labelled the number
as unsourced. Corrections were made in place, each marked, so the old wording can still be
seen next to the fix.

**What reproduced.** Almost everything:
- the power audit;
- the corpus, split and dedupe counts;
- the S50 locus table;
- S51's deltas and marginals;
- the Block 1 and S53r ladders;
- S54 Gate A/B, including the per-seed and checkpoint-swing figures;
- S55–S59, S64–S67 and the S60 tests.

The existing `audit_v4 --check` covered the headline recomputations. The rest was read from the
reports directly.

**Defects found, by how much they matter.**

| # | Where | What was wrong | Fix |
|---|---|---|---|
| 1 | S59 entry, model card, runbook, S60 payload | `contract.coverage` = **0.614 is 1 − referral rate** (selective coverage). All three documents called it "61.4% of bootstrap resamples satisfy all terms", which is impossible when every term's interval misses its target. | Wording fixed everywhere. The S60 field is renamed `joint_coverage` → `selective_coverage`. The `v4_s59_contract` family text is corrected. `audit_v4` now asserts `coverage == 1 − referral`. |
| 2 | S53r readout, model card §1/§4.2 | "V1 stays the base because Gate B found R1+R4 does not beat the V4 control". Gate B compares two V4 arms and cannot choose V1 over V4, and V4 pooled beats V1 by **+0.18 Macro-F1** on reserved (Gate A). The real reason is structural: S56 needs cross-fitted OOF predictions, which no V4 model has (`s59_plan.json:base`). | Reasoning corrected. Listed below as an open decision. |
| 3 | S56 body | Floor transfer given as "10 violated / 5 within CI"; the file says **12 / 3**, 0 met. | Fixed. |
| 4 | S49 table | Radius-6 chance rates (0.101%, 0.003%) and the "0.95×" had no file and do not reproduce. Recomputed: 0.0875% / 0.0015%, enrichment **1.08×**; 0.95× is the radius-3 either-rule. Observed counts (360,363 / 7,909 / 10,498) reproduce. | New artefact `results/v4/audit/dedupe_conventional_rule.json` (`audit_v4 --emit`); table corrected. Conclusion unchanged (≈1× chance). |
| 5 | S49 re-run, never recorded | S53r re-ran the archive probe with the fixed colour step (`results/v4/archive_probe_pre_post.json`, 2026-09-17 01:34 UTC). HAM val vs BCN decodability is 0.9905 → **0.9716**, Δ **−0.019**. The intervals do not overlap, but the change misses the probe's pre-existing −0.02 materiality rule, so the verdict flipped to **"not material"**. | Recorded here and in `v4_findings.md` §6.3. Reading: illuminant statistics carry a small, separable part of archive identity; 0.97 of it survives normalisation. |
| 6 | S52 Block 0 | "1.75 GB at batch 32, 384 px; 1.64–1.89 GB across arms" were readings with the backbone frozen. Banked fine-tune peaks are 2.41 GB (224 px) and 3.49 GB (384 px, 16×2); the 384 px batch-32 fine-tune anchor is 6.27 GB. The same entry also said both "batch 32 everywhere" and "16 at 384". | Annotated in place. |
| 7 | Frozen S62 index | `analysis_plan_v4.json` records `reserved_reads: 7`, because it counts only the sessions it indexes. There are **8** receipted reserved reads (S57a's specificity-only audit is the eighth), plus S51's probe, which predates receipts. | Index left untouched, because it is frozen and registered. `final_verdict_v4.json` records 8 with the reason; runbook, model card and findings now say 8. `audit_v4` asserts 8. |
| 8 | Rounding, found by the new manuscript audit | S58 H1−H0 pAUC **+0.020** (was +0.021); head recovery **63%** (was 62%); S59 under-40 deployed−V1 CI lower bound **0.310** (was 0.311). | Fixed in CHANGELOG, runbook, findings and manuscript. |
| 9 | Minor wording | S50 `<40` Δ +0.053 (was +0.052). S55 "60+ barely moved" (40-59 moved least). S57a "every arm within one SE" (A2/A4 are >1 SE worse) and "0.94–0.96 at 40–69" (0.935–0.957). S57a val "every curve arm lower" (A4 ties A1). Block 1 reference fine-tune SDs 0.0211/0.0216 not reproducible (0.0235/0.0301). S50 self-test-only figures labelled as such. S65 workload-matched paragraph made precise (the verdict stands: S56_matched matches under-40 sensitivity with fewer under-40 flags and more all-ages sensitivity). | Fixed in place. |
| 10 | Model card | Called 6,981 "lesions" (it is images; 5,229 lesions). "R2 re-run pending" was stale. It cited plan directories rather than plan files. Its §4.3 range mixed pAUC and AUC. | Fixed. |

**Logic caveat that cannot be edited away.** S48 froze the V4 primary endpoint as "under-40
escalation sensitivity at a referral budget matched to V1, lesion unit, MCID 0.10".
- S54 gated instead on under-40 pAUC@0.20 plus Macro-F1, which is the runbook's S54 wording.
  Neither S52 nor S54 recorded this as a deviation from S48.
- S56/S59 read sensitivity at nominal budgets, image unit.
- The S48 endpoint was therefore never read in exactly its declared form. The manuscript
  reframe says so (§II-D, Limitations).

**`research/v4/audit_v4.py` extended: 55 → 76 checks** (+3 artefact writes under `--emit`).
- S59 is added to `RECEIPTS`, closing the item S62 left open.
- New plan-hash checks for `s59_plan.json` and `analysis_plan_v4.json`.
- A check that the receipted reserved reads number exactly 8.
- S59 is recomputed from `policies_oof.json` on the frozen V1 reserved probabilities: per-band
  system sensitivity, referral, coverage = 1 − referral, the all-NOT_MET verdict and the
  deployed − V1 contrast.
- S53r is recomputed from the banked run JSONs: control seed SD and every rung's mean
  final-epoch delta.

**S63 — gated test read and manuscript reframe.**
- **Gate.** `research/v4/s63_final_verdict.py` (`--selftest` 6/6, `--check` PASS) reads the gate
  from S59's frozen report. No band meets its 0.855 floor (0.763 / 0.704 / 0.674), so the gate is
  **CLOSED**: no HAM test read, and the receipt stays at 2.
- **`results/v4/final_verdict_v4.json`** holds:
  - every V4 verdict, via the S62 index, with each plan/report hash re-verified;
  - what was not run, and why (S57c, S67 stages 0 and 2, the S66 V4 arm);
  - the 8 receipted reads;
  - the deployed stack and its contract, with coverage correctly defined;
  - four open decisions, each citing its evidence file.
- **Ledger.** One row, `v4_s63 / S63_final_verdict`, with prune-own. It was run twice: the diff
  is +1 line, and the ledger's CRLF bytes are preserved. The first attempt had rewritten the
  ledger's line endings; that was caught by the diff and reverted from a backup before the fix.
- **`paper/v4/manuscript_v4.tex`** is the reframe: *"Decision Layers Cannot Fix a Ranking
  Failure: A Pre-Registered Study of the Under-40 Melanoma Blind Spot in Dermoscopy Triage"*.
  - Framing: limits and accounting, as `v4_findings.md` recommended.
  - Contents: 4 tables, 1 figure (`under40_ceiling.png`), 18 references.
  - `paper/manuscript.tex` (V1) is untouched and `audit_manuscript` still passes.
- **`research/v4/audit_manuscript_v4.py`** checks **179 registered claims** against their files.
  Its closure check fails on any decimal it does not know. Both guard types were proved to fire
  on a mutated copy. `validate_structure.py --manuscript v4/manuscript_v4.tex` passes: 18 cites =
  18 bibitems, no dangling refs. It has never been compiled here (no LaTeX toolchain).
- **Document updates.**
  - `paper/v4/v4_findings.md` is finalised: the S53r table fills §5, §6.1 says 8 reads, and there
    are new §7 (the contract) and §8 (close-out).
  - Runbook session table: S59–S63 marked done.

**Open decisions V4 leaves** (`final_verdict_v4.json:open_decisions`):
1. **A V4 base for the deployed stack.** Needs GPU K-fold for cross-fitted predictions, plus a
   new plan. This is the only route with a plausible large effect on the contract's accuracy
   terms.
2. **Target-side threshold recalibration** on the BCN/MSKCC train rows already scored by V1.
   CPU only.
3. **A working smartphone gate.** HAM-only Mahalanobis or a modality classifier. CPU only.
4. **More under-40 escalating data.** The only lever the ranking results leave open.

Also still open and owner's call:
- S67 stage 0, a descriptive reserved read.
- Compiling the V4 manuscript in Overleaf.
- The 6.27 GB VRAM anchor in row 6 comes from the owner's hardware notes, not from a `results/`
  file.

**Verification at close.**
- `audit_v4 --check` 76/76.
- `audit_manuscript` (V1): all match.
- `audit_manuscript_v4`: 179 claims, 0 failures.
- `s62_freeze --check` PASS; `s63_final_verdict --check` PASS.
- `s60_contract --selftest` 11/11; `pytest tests/test_s60_service.py` 19/19.
- `validate_structure` passes for both manuscripts.
- `frozen_params --selftest` passes; the six frozen checkpoints are byte-identical.

### Phase Y — post-contract programme, sessions S68–S75 added (2026-09-17)

The V4 contract failed every term on reserved. Four open decisions recorded in `final_verdict_v4.json`:

1. A V4 base with cross-fitted OOF predictions (GPU K-fold + new plan) — only route with large accuracy effect.
2. Target-side threshold recalibration on BCN/MSKCC train rows (CPU, most likely to pull referral down).
3. A working smartphone admissibility gate (CPU, safety requirement).
4. More under-40 escalating data (long lead time, runs in parallel).

**S68–S75 added to V4_SESSION_RUNBOOK.md §6c.** Order: S68 and S69 run now (CPU). S70 is a checkpoint—requires your benchmark of the pooled corpus and identification of a fresh evaluation cohort. S71–S73 run only if S70 approves. S72 is GPU training (you run it). S74 finalizes. S75 (data sourcing) runs alongside. Reserved (8 reads) and test (2 reads) are frozen; S73 reads the fresh cohort once.

**Correction (same day).** The first draft of §6c quoted an unsourced pooled-corpus breakdown
("6,981 + 1,131 + 877") and a guessed "~3 nights" for S72. Replaced with sourced figures:
- The pooled train split is **15,294** images (`ml/data/manifest_v4.csv`), so about 12,235 per
  training fold in a 5-fold split.
- The S72 time estimate is extrapolated from the banked pooled runs (30 epochs, patience off):
  - R1+R4 composite: 4,624.5 s over 16 epochs → about **9.6 h** for 5 folds;
  - R0 control: 1,519.5 s over 15 epochs → about **3.4 h**.
- Both are extrapolations, and S70's benchmark replaces them.

`train_v4.py` has no fold option, so S72 cannot start before S71.

### S70 — checkpoint plan (2026-09-17)

**Planning session only.** No reserved or test read, and no code written yet.

**Audit findings**
- **No unread evaluation cohort exists on disk.**
  - `manifest_v4.csv` covers all 25,331 ISIC-2019 images.
  - V4 `train` holds only **81** under-40 escalating lesions (BCN 35 / HAM 34 / MSKCC 12), and
    `val` holds **0**.
  - Reserved's **104** remains the only cohort with enough under-40 escalating cases, and it has
    been read 8 times.
- **Benchmark (owner's terminal, RTX 5050, 12,235 images × 30 epochs).**

  | Setting | ms/batch | VRAM | Projected time |
  |---|---:|---:|---:|
  | 224 px, batch 32 | 102 | 2.41 GB | 22 min |
  | 384 px, batch 32 | 292 | 6.26 GB | 64 min |
  | 384 px, batch 16 | 144 | 3.38 GB | 63 min |

  - The benchmark uses synthetic tensors and does no JPEG decoding.
  - The banked pooled runs at 2 workers (`R0_pooled_s43`, `R1+R4_pooled_s43/s44`), scaled to one
    fold, give **~49 min (R0)** and **~75 min (R1+R4)**. At 224 px the pooled run is limited by
    decoding the large BCN/MSKCC images.
  - The benchmark figures are hand-copied until S70's build adds `--json-out` and they are
    re-recorded in `results/`.

**Owner decisions**
- **D1 — cohort.** A new external cohort, sourced by S75. S73's confirmatory read waits for it;
  an optional 9th read of reserved is descriptive only.
- **D2 — contract.** Keep the 0.855 floors and the 0.25 referral limit. Add a relative term:
  V4-based stack − V1 stack, under-40 system sensitivity at matched referral, MCID 0.05.
- **D3 — base recipe.** R0 control at 224 px, 5-fold, 30 epochs, patience off, about **4.1 h**
